...
canonical: "https://openfga.dev/pr-preview/pr-1362/docs/interacting/contextual-tuples"
...
When making requests to [Check](https://openfga.dev/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object), [BatchCheck](https://openfga.dev/api-reference/relationship-queries/send-a-list-of-%60check%60-operations-in-a-single-request), [ListObjects](https://openfga.dev/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with), [ListUsers](https://openfga.dev/api-reference/relationship-queries/list-the-users-matching-the-provided-filter-who-have-a-certain-relation-to-a-particular-type) and [Expand](https://openfga.dev/api-reference/relationship-queries/expand-all-relationships-in-userset-tree-format-and-following-userset-rewrite-rules-useful-to-reason-about-and-debug-a-certain-relationship) you can include contextual tuples in the request. These tuples are treated as if they were actual stored tuples during the evaluation of that request but are not persisted to the database and will not affect other requests.

## Common Use Cases

There are three main use cases for contextual tuples:

1. When you want to avoid synchronizing data to OpenFGA. This is a powerful use case, as it allows using OpenFGA in a hybrid mode, with some data written to the database and other data obtained before making an authorization query. A good example is using user group memberships [from an identity token issued by an identity provider](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/token-claims-contextual-tuples.md). Note that while it's possible to provide all data using contextual tuples without storing any data, this approach wouldn't leverage the main benefit of the Zanzibar approach: avoiding the need to look up all data required for authorization decisions.

2. When a user has multiple relationships with the same object, and you want to specify which relationship to consider. For example, if a user belongs to multiple organizations but is logged into only one of them, you can [use contextual tuples to specify which organization to consider](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/organization-context-authorization.md).

3. When using information that's only available at runtime, such as the [current time](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/contextual-time-based-authorization.md) or the user's location. This scenario is better served with [Conditional Relationships](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/conditions.md).
...
- [More](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/token-claims-contextual-tuples.md)
