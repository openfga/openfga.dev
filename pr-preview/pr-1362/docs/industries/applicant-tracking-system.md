...
canonical: "https://openfga.dev/pr-preview/pr-1362/docs/industries/applicant-tracking-system"
...
**Per-job hiring teams, not org-wide visibility.** Recruiter and hiring manager are relations on each `job`, not just org roles. A hiring manager working on the _Senior Backend Engineer_ req does not see candidates from the _Sales Director_ req — even though both are "hiring managers" in the org.
...
| Multi-tenant ATS SaaS                      | tenant-scoped types, see [multi-tenant SaaS](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/use-cases/multi-tenant-saas.md) |
...
- **EEO / sensitive demographic fields.** Gate via [conditions](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/conditions.md) or a separate `can_view_eeo` relation restricted to compliance staff, distinct from the general candidate viewer set.
- **Time-bound interviewer access.** Use [conditions](https://openfga.dev/pr-preview/pr-1362/pr-preview/pr-1362/docs/modeling/conditions.md) so an interviewer's access to a candidate expires after the debrief, instead of relying on a cleanup job.
