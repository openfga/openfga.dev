...
canonical: "https://openfga.dev/pr-preview/pr-1372/docs/modeling/organization-context-authorization"
...
To follow this guide, you should be familiar with some [OpenFGA Concepts](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md).
...
- A [Check Request](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-check-request): is a call to the OpenFGA check endpoint that returns whether the user has a certain relationship with an object.
...
- A [Contextual Tuple](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-are-contextual-tuples): a tuple that can be added to a check request, and only exist within the context of that particular request.

You also need to be familiar with:

- **Modeling Object-to-Object Relationships**: You need to know how to create relationships between objects and how that might affect a user's relationships to those objects. [Learn more →](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/modeling/building-blocks/object-to-object-relationships.md)
- **Modeling Multiple Restrictions**: You need to know how to model requiring multiple authorizations before allowing users to perform certain actions. [Learn more →](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/modeling/multiple-restrictions.md)
...
In order to add a restriction based on the current organization context, we will make use of OpenFGA configuration language's support for [intersection](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/configuration-language.md#the-intersection-operator) to specify that a user has to both have access _and_ be in the correct context in order to be authorized.
...
For Check calls, OpenFGA has a concept called "[Contextual Tuples](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-are-contextual-tuples)". Contextual Tuples are tuples that do **not** exist in the system state and are not written beforehand to OpenFGA. They are tuples that are sent alongside the Check request and will be treated as _if_ they already exist in the state for the context of that particular Check call. That means that Anne can be using two different sessions, each within a different organization context, and OpenFGA will correctly respond to each one with the correct authorization decision.
...
- Are only supported on the [Check API endpoint](https://openfga.dev/pr-preview/pr-1372/api/service#Relationship%20Queries/Check) and [ListObjects API endpoint](https://openfga.dev/pr-preview/pr-1372/api/service#Relationship%20Queries/ListObjects). They are not supported on read, expand and other endpoints.
- If you are using the [Read Changes API endpoint](https://openfga.dev/pr-preview/pr-1372/api/service#Relationship%20Tuples/ReadChanges) to build a permission aware search index, note that it will not be trivial to take contextual tuples into account.
...
- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/modeling/multiple-restrictions.md)

**Contextual and Time-Based Authorization**

Learn how to authorize access that depends on dynamic or contextual criteria.

- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/modeling/contextual-time-based-authorization.md)
