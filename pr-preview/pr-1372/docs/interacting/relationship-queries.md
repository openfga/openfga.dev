...
canonical: "https://openfga.dev/pr-preview/pr-1372/docs/interacting/relationship-queries"
...
In order to understand this guide correctly you must be familiar with some [OpenFGA Concepts](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md) and know how to develop the things that we will list below.

Assume that you have the following [authorization model](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-an-authorization-model).<br />You have a [type](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-type) called `document` that can have a `reader` and `writer`. All writers are readers. `bob` has a `writer` relationship with `document:planning`.
...
- A [Type](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-type): a class of objects that have similar characteristics
- A [User](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-user): an entity in the system that can be related to an object
- A [Relation](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-relation): is a string defined in the type definition of an authorization model that defines the possibility of a relationship between an object of the same type as the type definition and a user in the system
- An [Object](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-an-object): represents an entity in the system. Users' relationships to it can be define through relationship tuples and the authorization model
- A [Relationship Tuple](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/concepts.md#what-is-a-relationship-tuple): a grouping consisting of a user, a relation and an object stored in OpenFGA

## Check

### What is it for?

The [Check API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/Check) is an API endpoint that returns whether the user has a certain relationship with an object. OpenFGA will resolve all prerequisite relationships to establish whether a relationship exists.
...
The [Batch Check API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/BatchCheck) is an API endpoint that allows you to check multiple user-object-relationship combinations in a single request.
...
1. When determining if the user has access to a list of objects (such as [Option 1 in Search with Permissions](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/interacting/search-with-permissions.md#option-1-search-then-check)), filter and sort on your database, then call `/batch-check`. Repeat to perform pagination.
...
If you are making less than 10 checks, it may be faster to call the [Check API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/Check) in parallel instead of Batch Check.

## Read

### What Is It For?

The [Read API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Tuples/Read) is an API endpoint that returns the relationship tuples that are stored in the system that satisfy a query.
...
The [ListObjects API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#/Relationship%20Queries/ListObjects) is an API endpoint that returns the list of all the objects of a particular type that a specific user has a specific relationship with.

It provides a solution to the [Search with Permissions (Option 3)](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/interacting/search-with-permissions.md#option-3-build-a-list-of-ids-then-search) use case for access-aware filtering on small object collections.

### When to use?

Use the ListObjects API to get what objects a user can see based on the relationships they have. See [Search with Permissions](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/interacting/search-with-permissions.md) for more guidance.
...
- The [standard version](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/ListObjects), which waits until all results are ready and sends them in one response.
- The [streaming version](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/StreamedListObjects), which should be used if you want the individual results as soon as they become available.

### Caveats

ListObjects will return the results found within the time allotted (`listObjectsDeadline`, default: `3s`) up to the maximum number of results configured (`listObjectsMaxResults`, default: `1000`). See [Configuring the Server](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/docs/getting-started/setup-openfga/configure-openfga.md)) for more on how to change the default configuration.
...
The [ListUsers API](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#/Relationship%20Queries/ListUsers) is an API endpoint that that returns all users of a given type that have a specified relationship with an object.
...
- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/Check)

**Read API Reference**

Official reference guide for the Read API

- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Tuples/Read)

**Expand API Reference**

Official reference guide for the Expand API

- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/Expand)

**ListObjects API Reference**

Official reference guide for the ListObjects API

- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/ListObjects)

**ListUsers API Reference**

Official reference guide for the ListUsers API

- [More](https://openfga.dev/pr-preview/pr-1372/pr-preview/pr-1372/api/service#Relationship%20Queries/ListUsers)
