...
canonical: "https://openfga.dev/pr-preview/pr-1367/docs/modeling/getting-started"
content_type: "documentation"
last_updated: "2026-09-21T10:28:01.000Z"
---

# Get Started with Modeling

Creating a [Relationship Based Access Control (ReBAC)](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/authorization-concepts.md#what-is-relationship-based-access-control) authorization model might feel odd at first. Most of us tend to think about authorization models in terms of roles and permissions. After all, most software works like that. Your existing systems are likely built on a model using roles and permissions.

This guide outlines a process for defining your [authorization model](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-an-authorization-model) with [OpenFGA](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/fga.md).
...
OpenFGA is built to quickly and reliably make [authorization checks](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-check-request) . This means providing an answer to a question: "Can user U perform action A on object O?"

ReBAC systems determine access from a [user's](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-user) [relation](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-relation) to an [object](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-an-object). Authorization decisions are then yes or no answers to the question: "Does user U have relation R with object O?".
...
![The starting point](/pr-preview/pr-1367/assets/images/getting-started-diagram-01-55e7a873e7fdd4aa810922af9606ad0c.svg)

### 01. Pick The Most Important Feature

![Pick the most important feature](/pr-preview/pr-1367/assets/images/getting-started-diagram-02-46c38ef0a05b5eac7e8a85c3ca02e20f.svg)
...
Roles don't "disappear" in ReBAC systems like OpenFGA. Your users might [have roles on a given object, rather than the entire system](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/roles-and-permissions.md). But starting from the term "role" might lead you down the wrong path. Instead it is better to discover roles while you are modeling.

Your feature description should include the [objects](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-an-object), [users](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-user) and [groups of users](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-user) participating in the system. Sentences should look like this:
...
![List the object types](/pr-preview/pr-1367/assets/images/getting-started-diagram-03-3734cdad8827d03f6a76db672f13be46.svg)

Next make a list of the [types](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-type) of objects in your system. You might be able to identify the objects in your system from your existing domain/database model.
...
Now that we have a list of object types we can start defining them using the [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/../configuration-language):
...
In those cases [**User** should also be an object type](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/building-blocks/object-to-object-relationships.md). Following the last recommendation, we would discover the User type because it is a second noun in an expression: "friend of the user".

### 03. List Relations For Those Types

![List relations for those types](/pr-preview/pr-1367/assets/images/getting-started-diagram-04-e8b2110197173b6308a3f797e76aa471.svg)

Each of the previously defined types has a set of relations. [Relations](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-relation) are an important component in your model. After all, OpenFGA is a [Relationship Based Access Control (ReBAC)](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/authorization-concepts.md#what-is-relationship-based-access-control-rebac) system.
...
Using the [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/../configuration-language) we can enumerate the relations for each type:
...
![Define relations](/pr-preview/pr-1367/assets/images/getting-started-diagram-05-ffffd65d9b3eb843353c35f2f663a0c5.svg)

We will use the [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/../configuration-language) to create a [relation definition](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-relation) for each of the relations we identified. At this stage we will encode the answers to the question we asked at the beginning of the document:.
...
Relation definitions of the form "define {relation}: \[user, organization#member]" are fairly common. They are used to express that relationships "to the object with that relation" (e.g. "users" of type user or "member of organization") can be assigned by your system and that only the users that have that relation are those with a [direct relationship](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/building-blocks/direct-relationships.md).

You can read more about group membership and types in [Modeling User Groups](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/user-groups.md).
...
We will specify this logic as part of [directly related user type](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-directly-related-user-type).
...
If you want to learn more, you can read further about this in [Modeling User Groups](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/user-groups.md) and [Managing Relationships Between Objects](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/interacting/managing-relationships-between-objects.md).

##### Complete Type Definition

The complete [type definition](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-type-definition) for the **organization** type is:
...
In the current version, there is no way to state that there is only one owner in the authorization model. The application must limit this [set of users](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-user) to just one owner if that is a requirement.

When a document is created, a relationship tuple will be stored in OpenFGA representing this relationship between owner and document. This is an example of a [user to object relationship](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/direct-access.md).
...
Why? This [relation definition](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-relation) states that:
...
This also supports making all members in an organization editors of the document, through a [group to object relationship](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/user-groups.md). A relationship tuple like the following one states that the members of organization A are editors of document 0001.
...
You can learn more about this in [Modeling User Groups](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/user-groups.md).
...
This relation is different from the others we have seen so far, as it is a relation between two objects (a **folder** and or **drive** that is the parent of the **document**). This is known as an [object to object relationship](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/building-blocks/object-to-object-relationships.md), of which [parent-child is a particular case](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/parent-child.md).
...
We can use [direct type restriction](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#direct-relationship-type-restrictions) to ensure a document's parent can only be an object of type either drive or folder.

Side note

You might have noticed that the "user" in the tuple is an object. This is a special syntax OpenFGA accepts in the "user" parameter to write [object to object relationships](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/building-blocks/object-to-object-relationships.md). You can read more about writing data to manage object to object relationships in [Managing Relationships Between Objects](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/interacting/managing-relationships-between-objects.md).

##### Relation: can\_share

We need to express the following in the [relation definition](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-relation):
...
We can achieve that with the following definition using [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/../configuration-language):
...
- **We don't use a [direct relationship type restriction](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#direct-relationship-type-restrictions) as part of the definition.** can\_share is a common example of representing a permission that is defined in terms of other relations but is not directly assignable by the system.

- The relation definition contains a [union operator](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#the-union-operator) separating a list of relations that the user must have with the object in order to "be able to share the document". It is any of:
...
You can read more about the aforementioned items in [Modeling Roles and Permissions](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/roles-and-permissions.md).
...
Similar to the [can\_share relation](#relation-can_share), we can achieve that with the following definition using [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/../configuration-language):
...
The complete [type definition](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-a-type-definition) for the document type is:
...
![Test the model](/pr-preview/pr-1367/assets/images/getting-started-diagram-06-8eb4b8f4a925670b0ff05fa86c4b890e.svg)
...
![Iterate](/pr-preview/pr-1367/assets/images/getting-started-diagram-07-110858004c2892e28df4c404a50abce2.svg)
...
When defining the authorization model for your own system, you would continue iterating on the [authorization model](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md#what-is-an-authorization-model) with the [next feature](#01-pick-the-most-important-feature) and so on.
...
- [More](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/concepts.md)

**Configuration Language**

Learn about OpenFGA Configuration Language.

- [More](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md)

**Direct Access**

Learn about modeling user access to an object.

- [More](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/direct-access.md)
