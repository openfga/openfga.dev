...
canonical: "https://openfga.dev/pr-preview/pr-1367/docs/concepts"
...
The [OpenFGA](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/fga.md) service answers [authorization](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/authorization-concepts.md#authentication-and-authorization) [checks](#what-is-a-check-request) by determining whether a **[relationship](#what-is-a-relation)** exists between an [object](#what-is-an-object) and a [user](#what-is-a-user). Checks reference your **[authorization model](#what-is-an-authorization-model)** against your **[relationship tuples](#what-is-a-relationship-tuple)** for authorization authority. Below are explanations of basic FGA concepts, like type and authorization model, and a [playground](https://play.fga.dev/) to test your knowledge.
...
- A JSON syntax accepted by the OpenFGA API that closely follows the original syntax in the [Zanzibar Paper](https://research.google/pubs/pub48190/). For more information, see [Equivalent Zanzibar Concepts](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#equivalent-zanzibar-concepts).
...
Click here to learn more about the [OpenFGA Configuration Language](https://openfga.dev/pr-preview/pr-1367/configuration-language).
...
For an example, see [Direct Access](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/direct-access.md).
...
For more information, see [Direct Access](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/direct-access.md).
...
## What Are Direct And Implied Relationships?A **direct relationship** (R) between user X and object Y means the relationship tuple (user=X, relation=R, object=Y) exists, and the OpenFGA authorization model for that relation allows the direct relationship because of [direct relationship type restrictions](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#direct-relationship-type-restrictions).An **implied (or computed) relationship** (R) exists between user X and object Y if user X is related to an object Z that is in a direct or implied relationship with object Y, and the OpenFGA authorization model allows it.

- `user:anne` has a direct relationship with `document:new-roadmap` as `viewer` if the [type definition](#what-is-a-type-definition) allows it with [direct relationship type restrictions](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/configuration-language.md#direct-relationship-type-restrictions), and one of the following [relationship tuples](#what-is-a-relationship-tuple) exist:
...
Check requests use the `check` methods in the OpenFGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [check endpoint](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/Check) using curl or in your code. The check endpoint responds with `{ "allowed": true }` if a relationship exists, and with `{ "allowed": false }` if the relationship does not.
...
For more information, see the [Relationship Queries page](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/interacting/relationship-queries.md) and the official [Check API Reference](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/Check).
...
List objects requests are completed using the `listobjects` methods in the OpenFGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [list objects endpoint](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/ListObjects) using curl or in your code.
...
For more information, see the [Relationship Queries page](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/interacting/relationship-queries.md) and the [List Objects API Reference](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/ListObjects).
...
List users requests are completed using the relevant `ListUsers` method in SDKs, the `fga query list-users` command in the CLI, or by manually calling the [ListUsers endpoint](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/ListUsers) using curl or in your code.
...
For more information, see the [ListUsers API Reference](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/ListUsers).
...
For more information, see [Contextual and Time-Based Authorization](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/contextual-time-based-authorization.md), [Authorization Through Organization Context](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/organization-context-authorization.md) and [Check API Request Documentation](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/api/service#Relationship%20Queries/Check).
...
Note: `<type>:*` cannot be used in the `relation` or `object` properties. In addition, `<type>:*` cannot be used as part of a userset in the tuple's user field. For more information, see [Modeling Public Access](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/public-access.md) and [Advanced Modeling: Modeling Google Drive](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/modeling/advanced/gdrive.md).
...
- [More](https://openfga.dev/pr-preview/pr-1367/pr-preview/pr-1367/docs/authorization-concepts.md)
