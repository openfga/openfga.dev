...
${o.map(n=>y(n)).join(`,
`)}
]`}function oe(t,e,o=!1,n=!1){let i=t===c.PYTHON_SDK?"ClientTuple":`ClientTupleKey${o?"WithoutCondition":""}`;n&&(i="TupleKey");let r=Object.fromEntries(["user","relation","object"].map(s=>[s,m(e[s])]));if(e.condition){let s=t===c.JAVA_SDK?"ClientRelationshipCondition":t===c.GO_SDK?"openfga.RelationshipCondition":"RelationshipCondition",a={name:m(e.condition.name)};f(e.condition.context)&&(a.context=`${t===c.GO_SDK?"&":""}${A(t,e.condition.context)}`),r.condition=`${t===c.GO_SDK?"&":""}${C(t,s,a)}`}return C(t,i,r)}function re(t,e,o){let n={},i=s=>A(t,s),r=t===c.DOTNET_SDK&&e==="batchItem";for(let[s,a]of Object.entries(o))if(!["authorization_model_id","consistency"].includes(s))switch(s){case"tuple_key":Object.assign(n,Object.fromEntries(Object.entries(a).map(([l,d])=>[l,m(d)])));break;case"contextual_tuples":{let l=G(t,r?"TupleKey":"ClientTupleKey",(Array.isArray(a)?a:a.tuple_keys).map(u=>oe(t,u,!1,r))),d=t===c.JAVA_SDK&&["listObjects","listUsers"].includes(e)?"contextualTupleKeys":"contextualTuples";n[d]=r?C(t,"ContextualTupleKeys",{tupleKeys:l}):t===c.JS_SDK&&e==="batchItem"?`{"tuple_keys": ${l}}`:l;break}case"context":n.context=`${t===c.GO_SDK?"&":""}${i(a)}`;break;case"checks":n.checks=G(t,"ClientBatchCheckItem",a.map(l=>C(t,"ClientBatchCheckItem",re(t,"batchItem",l))));break;case"correlation_id":n.correlationId=m(a);break;case"writes":case"deletes":n[s]=G(t,`ClientTupleKey${s==="deletes"?"WithoutCondition":""}`,a.tuple_keys.map(l=>oe(t,l,s==="deletes")));break;case"object":n.object=e==="listUsers"&&t!==c.JS_SDK?C(t,t===c.GO_SDK?"openfga.FgaObject":"FgaObject",{type:m(a.type),id:m(a.id)}):i(a);break;case"user_filters":n[t===c.JS_SDK?"user_filters":"userFilters"]=G(t,t===c.GO_SDK?"openfga.UserTypeFilter":"UserTypeFilter",a.map(l=>C(t,t===c.GO_SDK?"openfga.UserTypeFilter":"UserTypeFilter",{type:m(l.type),...f(l.relation)?{relation:t===c.GO_SDK?`openfga.PtrString(${m(l.relation)})`:m(l.relation)}:{}})));break;default:n[s]=i(a)}return n}function Ce(t,e,o,n){let i={};if(f(o.authorization_model_id)&&(i.authorizationModelId=t===c.GO_SDK?`openfga.PtrString(${m(o.authorization_model_id)})`:m(o.authorization_model_id)),f(o.consistency)){let r=o.consistency.split("_").map(s=>s[0]+s.slice(1).toLowerCase()).join("");i.consistency=t===c.GO_SDK?`openfga.CONSISTENCYPREFERENCE_${o.consistency}.Ptr()`:t===c.JAVA_SDK?`ConsistencyPreference.${o.consistency}`:t===c.DOTNET_SDK?`ConsistencyPreference.${o.consistency.replaceAll("_","")}`:t===c.PYTHON_SDK?m(o.consistency):`ConsistencyPreference.${r}`}if(f(n.headers)&&(t===c.GO_SDK?i.requestOptions=`RequestOptions{Headers: map[string]string{${Object.entries(n.headers).map(([r,s])=>`${m(r)}: ${m(s)}`).join(", ")}}}`:t===c.DOTNET_SDK?i.headers=`new Dictionary<string, string> { ${Object.entries(n.headers).map(([r,s])=>`{ ${m(r)}, ${m(s)} }`).join(", ")} }`:t===c.JAVA_SDK?i.additionalHeaders=`Map.ofEntries(${Object.entries(n.headers).map(([r,s])=>`Map.entry(${m(r)}, ${m(s)})`).join(", ")})`:i.headers=A(t,n.headers)),e==="batchCheck"&&(i.maxBatchSize=t===c.GO_SDK?"openfga.PtrInt32(50)":"50",i.maxParallelRequests=t===c.GO_SDK?"openfga.PtrInt32(10)":"10",t===c.PYTHON_SDK&&(delete i.maxBatchSize,delete i.maxParallelRequests)),e==="write"&&f(n.conflictOptions)){let r={};for(let[s,a]of Object.entries(n.conflictOptions))r[s]=t===c.JS_SDK?`ClientWriteRequest${w(s)}.${w(a)}`:t===c.GO_SDK?`CLIENT_WRITE_REQUEST_${N(s).toUpperCase()}_${a.toUpperCase()}`:t===c.DOTNET_SDK?`${w(s)}.${w(a)}`:t===c.PYTHON_SDK?`ClientWriteRequest${w(s)}.${a.toUpperCase()}`:`WriteRequest${s==="onDuplicateWrites"?"Writes.OnDuplicate":"Deletes.OnMissing"}Enum.${a.toUpperCase()}`;t===c.JAVA_SDK?(r.onDuplicateWrites&&(i.onDuplicate=r.onDuplicateWrites),r.onMissingDeletes&&(i.onMissing=r.onMissingDeletes)):i.conflict=C(t,t===c.GO_SDK?"ClientWriteConflictOptions":"ConflictOptions",r)}return i}function we(t,e,o){if(t==="check")return f(e.allowed)?{allowed:e.allowed}:void 0;if(t==="listObjects")return f(e.expectedResults)?{objects:e.expectedResults}:void 0;if(t==="listUsers")return e.expectedResults;if(t==="batchCheck")return e.checks.every(n=>f(n.allowed))?[c.CURL,c.GO_SDK,c.RPC].includes(o)?{result:Object.fromEntries(e.checks.map(n=>[n.correlation_id,{allowed:n.allowed}]))}:{result:e.checks.map(n=>({correlationId:n.correlation_id,allowed:n.allowed,request:{user:n.user,relation:n.relation,object:n.object,...f(n.contextualTuples)?{contextualTuples:{tuple_keys:F(n.contextualTuples)}}:{},...f(n.context)?{context:n.context}:{}}}))}:void 0}function B(t,e){let o=[c.PYTHON_SDK,c.CLI,c.CURL,c.PLAYGROUND].includes(e)?"#":"//";return t.split(/\r\n|[\r\n\u2028\u2029]/u).map(n=>`${o} ${n}`).join(`
`)}function Ae(t,e,o){let n=we(t,e,o);return f(n)?`

${B(`Expected response: ${g(n)}`,o)}`:t==="batchCheck"?e.checks.filter(i=>f(i.allowed)).map(i=>`
${B(`Expected allowed for ${m(i.correlation_id)}: ${i.allowed}`,o)}`).join(""):""}function Re(t,e){return[...t.relationshipTuples??[],...t.deleteRelationshipTuples??[],...t.contextualTuples??[],...t.checks??[],...(t.checks??[]).flatMap(o=>o.contextualTuples??[])].filter(o=>f(o._description)).map(o=>`${B(`${o.user} ${o.relation} ${o.object}
${$(o._description,"_description")}`,e)}
`).join("")}function ke(t,e,o,n){let i={check:"check",batchCheck:"batch-check",write:"write",listObjects:"list-objects",listUsers:"list-users",createStore:""}[t],r=b(JSON.stringify(e,null,2));return n&&t!=="createStore"&&!e.authorization_model_id&&(r=`${b(`{
...
# Custom headers are not supported by the CLI; use an SDK or curl.`:""}`}function x(t,e,o={},{environmentModelId:n=!1}={}){T(j[t],[e]);let i=E(t,o,{environmentModelId:n}),r=Ae(t,o,e),s=Re(o,e);if(e===c.CURL)return s+ke(t,i,o,n)+r;if(e===c.CLI)return s+De(t,i,o)+r;if(e===c.PLAYGROUND)return`is ${o.user} related to ${o.object} as ${o.relation}?${["contextualTuples","context","headers","consistency"].filter(k=>f(o[k])).map(k=>`
# ${k} is not supported on the playground; use an SDK or curl.`).join("")}${r}`;if(e===c.RPC)return`${s}${t}(${JSON.stringify(i,null,2)});${o.headers?`
// Request headers: ${g(o.headers)}`:""}${r}`;let a=w(t),l=re(e,t,i),d=Ce(e,t,i,o),u=t==="createStore"&&[c.PYTHON_SDK,c.JAVA_SDK].includes(e)?"CreateStoreRequest":`Client${a}Request`,p=C(e,u,l),_=e===c.PYTHON_SDK?`{
${Object.entries(d).map(([k,Se])=>y(`${m(N(k))}: ${Se},`)).join(`
`)}
...
`)}
)`,Te={WriteAuthorizationModelRequest:{required:["schema_version","type_definitions"],fields:{schema_version:"string",type_definitions:"TypeDefinition[]"}},TypeDefinition:{required:["type"],fields:{type:"string",relations:"Userset{}",metadata:"Metadata"}},Userset:{required:["this"],fields:{this:"empty"}},Metadata:{required:[],fields:{relations:"RelationMetadata{}"}},RelationMetadata:{required:[],fields:{directly_related_user_types:"RelationReference[]"}},RelationReference:{required:["type"],fields:{type:"string"}}},Q=t=>t.split(`
`).map(e=>`    ${e}`).join(`
`),R=(t,e)=>{throw new Error(`Unsupported Python authorization model at ${t}: ${e}`)},ge=(t,e)=>{(!t||Object.getPrototypeOf(t)!==Object.prototype)&&R(e,"expected an object")};function V(t,e,o,n="model"){if(t==="string")return(typeof e!="string"||!e.trim())&&R(n,"expected a nonempty string"),J(e);if(t.endsWith("[]"))return Array.isArray(e)||R(n,"expected an array"),e.length?`[
${e.map((s,a)=>Q(`${V(t.slice(0,-2),s,o,`${n}[${a}]`)},`)).join(`
`)}
]`:"[]";if(ge(e,n),t==="empty")return Object.keys(e).length&&R(n,"expected an empty object"),"{}";if(t.endsWith("{}"))return Object.keys(e).length?`{
${Object.entries(e).map(([s,a])=>Q(`${J(s)}: ${V(t.slice(0,-2),a,o,`${n}.${s}`)},`)).join(`
`)}
}`:"{}";let{required:i,fields:r}=Te[t];for(let s of Object.keys(e))Object.hasOwn(r,s)||R(`${n}.${s}`,"field is not supported by this generator");for(let s of i)Object.hasOwn(e,s)||R(n,`missing ${s}`);return o.add(t),`${t}(
${Object.entries(e).map(([s,a])=>Q(`${s}=${V(r[s],a,o,`${n}.${s}`)},`)).join(`
`)}
)`}function ce(t,e){S(t,e);let o=[],n=[],i={},r;switch(["ListStores","ReadAuthorizationModels","Read","ReadChanges"].includes(t)&&(i.page_size=e.pageSize,e.continuationToken!==void 0&&(i.continuation_token=e.continuationToken),t==="ListStores"&&e.name!==void 0&&(i.name=e.name)),t){case"ListStores":case"GetStore":case"DeleteStore":case"ReadAuthorizationModels":case"ReadAuthorizationModel":case"ReadAssertions":break;case"WriteAuthorizationModel":{let d=new Set;r=V("WriteAuthorizationModelRequest",e.model,d),o.push(`from openfga_sdk import ${[...d].join(", ")}`);break}case"Read":o.push("from openfga_sdk import ReadRequestTupleKey"),r=M("ReadRequestTupleKey",{user:e.user,relation:e.relation,object:e.object});break;case"ReadChanges":o.push("from openfga_sdk.client.models import ClientReadChangesRequest"),r=M("ClientReadChangesRequest",{type:e.objectType,...e.startTime!==void 0?{start_time:e.startTime}:{}});break;case"Expand":o.push("from openfga_sdk.client.models import ClientExpandRequest"),r=M("ClientExpandRequest",{relation:e.relation,object:e.object});break;case"WriteAssertions":o.push("from openfga_sdk.client.models import ClientAssertion"),r=`[
...
`)}}function le(t,e){if(!new Set(["ListStores","GetStore","DeleteStore","ReadAuthorizationModels","ReadAuthorizationModel","WriteAuthorizationModel","Read","ReadChanges","Expand","ReadAssertions","WriteAssertions","StreamedListObjects"]).has(t))throw new Error(`Unsupported Java API operation: ${t}`);S(t,e);let n=d=>JSON.stringify(d).replace(/[\u0085\u2028\u2029]/g,u=>`\\u${u.charCodeAt(0).toString(16).padStart(4,"0")}`),i=d=>`dev.openfga.sdk.api.configuration.${d}`,r=(d,u)=>`new ${d}()${Object.entries(u).map(([p,_])=>`
...
fgaClient.streamedListObjects(body, item -> System.out.println(item.getObject())).get();`)}}function de(t,e){if(!new Set(["ListStores","GetStore","DeleteStore","ReadAuthorizationModels","ReadAuthorizationModel","WriteAuthorizationModel","Read","ReadChanges","Expand","ReadAssertions","WriteAssertions","StreamedListObjects"]).has(t))throw new Error(`Unsupported .NET API operation: ${t}`);S(t,e);let n=l=>JSON.stringify(l).replace(/[\u0085\u2028\u2029]/g,d=>`\\u${d.charCodeAt(0).toString(16).padStart(4,"0")}`),i=(l,d)=>`new ${l} {
...
}`,"System")}}var X=t=>`'${t.replaceAll("'","'\\''")}'`,ue=({user:t,relation:e,object:o})=>({user:t,relation:e,object:o});function fe(t,e){S(t,e);let{method:o,path:n,viewer:i}=L(t),r={};P.includes(t)&&t!=="Read"&&(r.page_size=e.pageSize,e.continuationToken&&(r.continuation_token=e.continuationToken)),t==="ListStores"&&e.name&&(r.name=e.name),t==="ReadChanges"&&(r.type=e.objectType,e.startTime&&(r.start_time=e.startTime));let s;if(i){let a=Object.keys(j).find(l=>j[l]===i);s=E(a,{...e,authorizationModelId:""})}return t==="Read"&&(s={tuple_key:ue(e),page_size:e.pageSize,...e.continuationToken?{continuation_token:e.continuationToken}:{}}),t==="Expand"&&(s={tuple_key:{relation:e.relation,object:e.object}}),t==="WriteAuthorizationModel"&&(s=e.model),t==="WriteAssertions"&&(s={assertions:e.assertions.map(({expectation:a,...l})=>({tuple_key:l,expectation:a}))}),t==="StreamedListObjects"&&(s={user:e.user,relation:e.relation,type:e.objectType}),e.body&&(s=e.body),{method:o,path:n,query:r,...s?{body:s}:{}}}function Ee(t,e){let o=fe(t,e),n=o.path.replace("{store_id}","$FGA_STORE_ID").replace("{id}","$FGA_MODEL_ID").replace("{authorization_model_id}","$FGA_MODEL_ID"),i=Object.entries(o.query).map(([s,a])=>`${s}=${encodeURIComponent(a)}`).join("&"),r=o.body?X(JSON.stringify(o.body,null,2)):"";return["Expand","StreamedListObjects"].includes(t)&&(r=`${X(`{
  "authorization_model_id": "`)}"$FGA_MODEL_ID"${X(`",${JSON.stringify(o.body,null,2).slice(1)}`)}`),`curl${t==="StreamedListObjects"?" --no-buffer":""} -X ${o.method.toUpperCase()} "$FGA_API_URL${n}${i?`?${i}`:""}"${r?` \\
  -H "content-type: application/json" \\
  -d ${r}`:""}`}function xe(t,e){let o=t[0].toLowerCase()+t.slice(1),n=P.includes(t)?{pageSize:e.pageSize,...e.continuationToken?{continuationToken:e.continuationToken}:{},...t==="ListStores"&&e.name?{name:e.name}:{}}:{},i;if(t==="Read"&&(i=ue(e)),t==="ReadChanges"&&(i={type:e.objectType,...e.startTime?{startTime:e.startTime}:{}}),t==="Expand"&&(i={relation:e.relation,object:e.object}),t==="WriteAssertions"&&(i=e.assertions),t==="WriteAuthorizationModel"&&(i=e.model),t==="StreamedListObjects")return`const body = ${JSON.stringify(fe(t,e).body,null,2)};
...
${n?n.join(`
`):i?"from openfga_sdk.models import CreateStoreRequest":`from openfga_sdk.client.models import ${Ie[e]}${e==="ListUsersRequestViewer"?`
...
var fgaClient = new OpenFgaClient(config);`;default:throw new Error(`No SDK setup for language "${t}"`)}}function Ge(t,e="FGA Demo Store"){return te(t,"CreateStoreViewer",{storeName:e})}var K=(t,e=4)=>t.split(`
`).map(o=>`${" ".repeat(e)}${o}`).join(`
`);function te(t,e,o={}){let n=Object.keys(j).find(s=>j[s]===e);if(!n)throw new Error(`Unknown request viewer: ${e}`);let i=pe(t,e),r=x(n,t,{...o,authorizationModelId:o.authorizationModelId??""},{environmentModelId:!0});return be(t,i,r)}function be(t,e,o){if(t===c.JS_SDK)return`${e}
...
${o}`}function Ne(t,e,o={}){let{viewer:n,scope:i}=L(e);if(S(e,o),t!==c.CURL&&!ee[e]?.[t]?.method)throw new Error(`No named SDK operation for ${e}/${t} in the audited version`);if(n)return te(t,n,o);let{code:r,imports:s}=me(e,t,o),l=`${t===c.CURL&&Object.values(ee[e]).every(({method:d})=>d===null)?`# HTTP only: no named client or generated low-level operation in the audited SDK versions.
`:""}${$e(t,void 0,i,s)}`;return be(t,l,r)}globalThis.openfgaViewer=ne;})();
