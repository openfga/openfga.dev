...
const ARTIFACT_PATH = path.join(REPO_ROOT, 'docs-site/openfga-dsl-highlight.js');
const CODEGEN_PATH = path.join(REPO_ROOT, 'docs-site/fga-codegen.js');
const BUILD_SCRIPT = path.join(REPO_ROOT, 'docs-site/scripts/build-fga-codegen.sh');
const GLOBAL_CSS_PATH = path.join(REPO_ROOT, 'docs-site/global.css');
const AUTHZ_VIEWER_PATH = path.join(REPO_ROOT, 'docs-site/snippets/AuthzModelSnippetViewer.jsx');
const CODE_BLOCK_PATH = path.join(REPO_ROOT, 'docs-site/snippets/OpenFGACodeBlock.jsx');
...
].map((fileName) => path.join(REPO_ROOT, 'docs-site/snippets', fileName));
