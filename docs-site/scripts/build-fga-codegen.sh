...
# See docs-site/README.md for architecture and maintenance details.
...
TMP_DIR="$(mktemp -d "$REPO_ROOT/docs-site/.codegen.XXXXXX")"
...
NODE_PATH="$REPO_ROOT/node_modules" node_modules/.bin/esbuild \
  docs-site/scripts/openfga-dsl-highlight.entry.cjs \
  --bundle \
  --format=iife \
  --platform=browser \
  --target=es2020 \
...
node_modules/.bin/esbuild docs-site/scripts/viewer-runtime.entry.mjs \
  --bundle \
  --format=iife \
  --platform=browser \
  --target=es2020 \
...
if [[ "$CHECK_ONLY" == true ]]; then
  ...
  for artifact in fga-codegen.js openfga-dsl-highlight.js openfga-viewer.js; do
    if ! cmp -s "$TMP_DIR/$artifact" "docs-site/$artifact"; then
      echo "docs-site/$artifact is stale; run npm run generate:mintlify-codegen" >&2
      STALE=1
    fi
  done
  if [[ "$STALE" -ne 0 ]]; then
...
mv "$TMP_DIR/fga-codegen.js" docs-site/fga-codegen.js
mv "$TMP_DIR/openfga-dsl-highlight.js" docs-site/openfga-dsl-highlight.js
mv "$TMP_DIR/openfga-viewer.js" docs-site/openfga-viewer.js

FGA_SIZE="$(wc -c < docs-site/fga-codegen.js)"
HIGHLIGHT_SIZE="$(wc -c < docs-site/openfga-dsl-highlight.js)"
echo "Generated docs-site/fga-codegen.js: $(( FGA_SIZE / 1024 ))KB"
echo "Generated docs-site/openfga-dsl-highlight.js: $(( HIGHLIGHT_SIZE / 1024 ))KB"
