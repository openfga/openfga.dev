...
import { componentFixturePath, previousComponentFixturePath, previousDocsPath } from './component-fixtures.mjs';
import {
  analyzeMdx,
  compareWithRef,
  encodeOpenFgaCode,
...
test('compare-ref follows the explicit native root rename without losing model coverage', (t) => {
  ...
  git('init', '--quiet');
  git('add', '.');
  git('-c', 'user.name=OpenFGA Test', '-c', 'user.email=test@openfga.dev', 'commit', '--quiet', '-m', 'before rename');
  mkdirSync(path.dirname(docs), { recursive: true });
  renameSync(previous, docs);
  const compare = () => compareWithRef('HEAD', { docsRoot: docs, repoRoot: root, logger() {} });
  assert.equal(compare().comparedFiles, 1);
  writeFileSync(path.join(docs, 'example.mdx'), `${IMPORT}\n\n${component('type changed')}\n`);
  assert.throws(compare, /drifted from HEAD/);
  unlinkSync(path.join(docs, 'example.mdx'));
  assert.throws(compare, /no current counterpart/);
});
