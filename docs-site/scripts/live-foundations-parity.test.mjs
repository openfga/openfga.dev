...
test('foundation fixtures cover each of the 26 assigned source pages exactly once', () => {
  const manifest = JSON.parse(readFileSync(new URL('docs-site/source-pages.json', root), 'utf8'));
...
for (const page of pages) test(`live foundation prose and examples: ${page.source}`, () => {
  assertPage(page, readFileSync(new URL(`docs-site/${page.native}`, root), 'utf8'));
});
...
for (const page of headingPages) test(`published foundation headings: ${page.source}`, () => {
  assertPublishedHeadings(page, readFileSync(new URL(`docs-site/${page.native}`, root), 'utf8'));
});

const concepts = pages.find((page) => page.source === 'concepts.mdx');
const conceptsSource = readFileSync(new URL(`docs-site/${concepts.native}`, root), 'utf8');
...
const frameworkSource = readFileSync(new URL(`docs-site/${frameworkHeadings.native}`, root), 'utf8');
