...
function fixture(t) {
  ...
  const save = () => {
    ...
    put('docs-site/docs.json', JSON.stringify(docs));
  };
...
mutation(
  ...
  ({ root }) => {
    symlinkSync(join(root, 'src/pages'), join(root, 'docs-site/other'));
  },
  /docs-site\/other: symlinks are not allowed/,
);
...
test('malformed JSON and invalid CLI options fail with context', (t) => {
  ...
  assert.match(result.stderr, /docs-site\/source-pages.json:/);
