    ...
    if (typeof value === 'string') {
      ...
      if (/^(?:https?:)?\/\//i.test(value)) {
        ...
        try {
          ...
          throw new Error(`docs-site/docs.json ${location}: invalid navigation URL ${value}`, { cause: error });
        }
...
export function validateSourceCoverage({ repoRoot = repositoryRoot, logger = console.log } = {}) {
  ...
  for (const { page, location, hidden } of references) {
    ...
    if (prohibitedPages.has(page)) {
      problems.push(
        `docs-site/docs.json ${location}: ${page} must not enter production navigation (${prohibitedPages.get(page)})`,
      );
    } else if (!ownedPages.includes(page)) {
      problems.push(`docs-site/docs.json ${location}: ${page} is not a mapped Mintlify-owned page`);
    }
    if (!hidden) visibleCounts.set(page, (visibleCounts.get(page) ?? 0) + 1);
  }
  for (const [page, count] of allCounts) {
    if (count > 1)
      problems.push(`docs-site/docs.json: duplicate navigation reference ${page} (${count} occurrences)`);
  }
  for (const page of ownedPages) {
    if (!visibleCounts.has(page))
      problems.push(`docs-site/docs.json: ${page} is missing from visible documentation navigation`);
  }
