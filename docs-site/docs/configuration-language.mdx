...
OpenFGA's Configuration Language builds a representation of a system's [authorization model](/docs/concepts#what-is-an-authorization-model), which informs [OpenFGA's API](/api-reference) on the [object types](/docs/concepts#what-is-a-type) in the system and how they relate to each other. The Configuration Language describes the [relations](/docs/concepts#what-is-a-relation) possible for an object of a given type and lists the conditions under which one is related to that object.
...
When used at the beginning of a [relation definition](/docs/concepts#what-is-a-relation-definition), `[<string>, <string>, ...]` allows [direct relationships](/docs/concepts#what-are-direct-and-implied-relationships) by the objects of these specified types, for example, `[user, user:*, team#member]`. The strings can be in one of three formats:
...
* Anne is a viewer of the parent folder of the new-roadmap document
  ...
  1. planning folder is the parent folder of the new-roadmap document
  2. anne is a viewer of the planning folder
