...
You can list all the authorization models for a store using the [ReadAuthorizationModels](/api/service/authorization-models/return-all-the-authorization-models-for-a-particular-store) API. This endpoint returns the results sorted in reverse chronological order, as in the first model in the list is the latest model. By default, only the last 50 models are returned, but you can paginate across by passing in the appropriate `continuation_token`.

## How to target a particular model

Some endpoints relating to tuples ([Check](/api/service/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object), [ListObjects](/api/service/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with), [ListUsers](/api/service/relationship-queries/list-the-users-matching-the-provided-filter-who-have-a-certain-relation-to-a-particular-type), [Expand](/api/service/relationship-queries/expand-all-relationships-in-userset-tree-format-and-following-userset-rewrite-rules-useful-to-reason-about-and-debug-a-certain-relationship), [Write](/api/service/relationship-tuples/add-or-delete-tuples-from-the-store)) accept an `authorization_model_id`, which we strongly recommend passing, especially in production.
