---
title: "\U0001F6E1\uFE0FSetup Access Control"
sidebarTitle: Setup Access Control
...
<Warning>
The built-in access control feature in OpenFGA is experimental and is not recommended for production use. We are looking for feedback on this, so if you do try it, please reach out on our [openfga Slack channel](https://cloud-native.slack.com/archives/C06G1NNH47N) in the CNCF community.
</Warning>
...
- OIDC Provider: You need to have an OIDC provider [set up and configured](./configure-openfga#oidc) in your OpenFGA server set up to use access control.
...
\x20 relations
\x20   define admin: [application] # required
\x20   define can_call_create_stores: admin # required
\x20   define can_call_list_stores: [application, application:*] or admin # required

type application # required

type store # required
\x20 relations
\x20   define system: [system] # required
\x20   define admin: [application] or admin from system # required
\x20   define model_writer: [application] or admin
\x20   define reader: [application] or admin
\x20   define writer: [application] or admin
\x20   define can_call_delete_store: admin # required
\x20   define can_call_get_store: reader or writer or model_writer # required
\x20   define can_call_check: reader # required
\x20   define can_call_expand: reader # required
\x20   define can_call_list_objects: reader # required
\x20   define can_call_list_users: reader # required
\x20   define can_call_read: reader # required
\x20   define can_call_read_assertions: reader or model_writer # required
\x20   define can_call_read_authorization_models: reader or model_writer # required
\x20   define can_call_read_changes: reader # required
\x20   define can_call_write: writer # required
\x20   define can_call_write_assertions: model_writer # required
\x20   define can_call_write_authorization_models: model_writer # required

type module # required
\x20 relations
\x20   define store: [store] # required
\x20   define writer: [application]
\x20   define can_call_write: writer or writer from store # required`} />
...
2. Grant access to writing tuples of a certain module in a store.
   ...
   In order to grant access to only write to relations in certain modules, you must have a model with modules. Refer to the [modular models documentation](../../modeling/modular-models) for more on that feature.
...
<CardGroup>
  <Card title="Setup OpenFGA" href="./configure-openfga">
    Learn how to setup and configure an OpenFGA server
  </Card>
  <Card title="Setup OIDC" href="./configure-openfga#oidc">
    Learn how to setup and configure an OpenFGA server
  </Card>
