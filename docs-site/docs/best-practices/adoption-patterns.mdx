...
sidebarTitle: "Adoption Patterns"
---
...
<OpenFGACodeBlock code={`model\x20
\x20 schema 1.1
...
\x20     # .. add additional organization roles

\x20     # map permissions to organization roles\x20
\x20     define can_add_member: admin
\x20     define can_delete_member: admin
\x20     define can_view_member: admin or member
\x20     define can_add_resource: admin or member`} />
...
\x20     # map resource permissions to organization roles
\x20      define can_delete_resource: admin from organization or member from organization
\x20      define can_view_resource: admin from organization or member from organization\x20\x20\x20\x20\x20\x20`} />
...
<OpenFGACodeBlock code={`\x20  type resource
\x20    relations
\x20      define organization: [organization]
\x20      define owner: [user]
\x20      define viewer: [user]

\x20     # map resource permissions to organization roles
\x20      define can_delete_resource: admin from organization or member from organization or owner
\x20      define can_view_resource: admin from organization or member from organization or owner or viewer`} />
...
<h2 id="use--to-enrich-jwts">Use OpenFGA to enrich JWTs</h2>

Once you have your authorization model and data set up, you can start making authorization checks from your application. The preferred way is to perform a [Check()](/docs/getting-started/perform-check) call.
...
<OpenFGACodeBlock code={`\x20 type user
\x20 type organization
\x20   relations
\x20     define member: [user]`} />
...
- [Modular Models](/docs/modeling/modular-models) enable each team to independently evolve their authorization policies without relying on a central team.
- [Access Control](/docs/getting-started/setup-openfga/access-control) allows you to issue different credentials for each application, with permissions that ensure that each credential can only write data to the types defined in the Modules they own.
...
<h2 id="shadowing-the--api">Shadowing the OpenFGA API</h2>
...
4. Once confident in the results, switch to using OpenFGA as the source of truth. The same approach of shallow checks when [migrating between models](/docs/getting-started/immutable-models#potential-use-cases).
