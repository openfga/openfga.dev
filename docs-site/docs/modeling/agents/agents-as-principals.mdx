...
This is orthogonal to patterns like [Task-Based Authorization](/docs/modeling/agents/task-based-authorization). Even if you use task-scoped permissions, you still want to constrain what an agent can do in general - beyond the permissions required for a specific task.
...
\x20   define member: [user, agent]
type project
\x20 relations
\x20   define organization: [organization]
\x20   define owner: [user]
\x20   define member: [user, agent]
\x20   define can_delete: owner or admin from organization
\x20   define can_edit: owner or admin from organization
\x20   define can_read: can_edit or member or member from organization
\x20   define can_create_issue: can_edit
type issue
\x20 relations
\x20   define project: [project]
\x20   define reporter: [user, agent]
\x20   define assignee: [user, agent]
\x20   define can_delete: reporter or can_delete from project
\x20   define can_edit: assignee or can_edit from project
\x20   define can_read: reporter or can_edit or can_read from project`} />
...
4. **Check permissions** using the same API calls you use for users - just with `agent:` as the subject. If you want to check for both user and agent permissions, perform a [batch check](/docs/interacting/relationship-queries#batch-check) call.
