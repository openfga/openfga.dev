...
- Are only supported on the [Check API endpoint](/api/service/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object) and [ListObjects API endpoint](/api/service/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with). They are not supported on read, expand and other endpoints.
- If you are using the [Read Changes API endpoint](/api/service/relationship-tuples/return-a-list-of-all-the-tuple-changes) to build a permission aware search index, note that it will not be trivial to take contextual tuples into account.
</Warning>
...
<CardGroup>
  ...
  <Card title="OpenFGA API" href="/api/service/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object">
