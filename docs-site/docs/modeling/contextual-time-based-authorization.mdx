...
sidebarTitle: "Contextual and Time-Based Authorization"
...
To follow this guide, you should be familiar with some [OpenFGA Concepts](/docs/concepts).

<h3 id="-concepts">OpenFGA concepts</h3>

- A **[Relation](/docs/concepts#what-is-a-relation)**: Defined in the type definition of an authorization model, a relation is a string that defines the possibility of a relationship between an object of the same type as the type definition and a user in the system.
- A **[Check Request](/docs/concepts#what-is-a-check-request)**: is a call to the OpenFGA check endpoint that returns whether the user has a certain relationship with an object.
...
- A **[Contextual Tuple](/docs/concepts#what-are-contextual-tuples)**: a tuple that can be added to a Check request, and only exists within the context of that particular request.

You also need to be familiar with:

- **Modeling Object-to-Object Relationships**: You need to know how to create relationships between objects and how that might affect a user's relationships to those objects. [Learn more →](/docs/modeling/building-blocks/object-to-object-relationships)
- **Modeling Multiple Restrictions**: You need to know how to model requiring multiple authorizations before allowing users to perform certain actions. [Learn more →](/docs/modeling/multiple-restrictions)
...
##### <span id="extend-the-authorization-model" style={{ scrollMarginTop: '7rem' }}>Extend the authorization model</span>
...
##### <span id="add-the-required-tuples-to-mark-that-anne-is-in-an-approved-context" style={{ scrollMarginTop: '7rem' }}>Add the required tuples to mark that Anne is in an approved context</span>
...
For Check calls, OpenFGA has a concept called "[Contextual Tuples](/docs/concepts#what-are-contextual-tuples)". Contextual Tuples are tuples that do **not** exist in the system state and are not written beforehand to OpenFGA. They are tuples that are sent alongside the Check request and will be treated as _if_ they already exist in the state for the context of that particular Check call.
...
<Accordion title="Final version of the Authorization Model and Relationship tuples">
...
- Are only supported on the [Check API endpoint](/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object) and [ListObjects API endpoint](/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with). They are not supported on read, expand and other endpoints.
- If you are using the [Read Changes API endpoint](/api-reference/relationship-tuples/return-a-list-of-all-the-tuple-changes) to build a permission aware search index, note that it will not be trivial to take contextual tuples into account.
</Warning>
...
<CardGroup>
  ...
  <Card title="Modeling with Multiple Restrictions" href="/docs/modeling/multiple-restrictions">
    Learn how to model requiring multiple relationships before users are authorized to perform certain actions.
  </Card>
  <Card title="OpenFGA API" href="/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object">
