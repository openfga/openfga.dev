...
sidebarTitle: "Authorization Through Organization Context"
...
- A **[Contextual Tuple](/docs/concepts#what-are-contextual-tuples)**: a tuple that can be added to a check request, and only exist within the context of that particular request.
...
In order to add a restriction based on the current organization context, we will make use of OpenFGA configuration language's support for [intersection](/docs/configuration-language#the-intersection-operator) to specify that a user has to both have access _and_ be in the correct context in order to be authorized.
...
<CardGroup>
  ...
  <Card title="OpenFGA Check API" href="/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object">
