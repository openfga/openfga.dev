...
sidebarTitle: "Parent-Child Objects"
...
In OpenFGA, a user's [relationship](/docs/concepts#what-is-a-relationship) with an [object](/docs/concepts#what-is-an-object) can affect their relationship with another object. For example, an `editor` of a `folder` can also be an `editor` of all `documents` that `folder` is a `parent` of.

<Note>
**When to use**

Object-to-object relationships can combine with a configured authorization model to indicate that a user's relationship with one object may influence the user's relationship with another object. They can also eliminate the need to modify relationships between objects using [user groups](/docs/modeling/user-groups#03-assign-the-team-members-a-relation-to-an-object).
...
Familiarize yourself with basic [OpenFGA Concepts](/docs/concepts):
...
<h3 id="01-update-the-athorization-model-to-allow-a-parent-relationship-between-folder-and-document">01. Update the Athorization Model to allow a parent relationship between folder and document</h3>

As documented in [Modeling Concepts: Object to Object Relationships](/docs/modeling/building-blocks/object-to-object-relationships), the following update to the authorization model allows a `parent` relation between a `folder` and a `document`:
...
<h3 id="04-create-a-new-relationship-tuple-to-indicate-that-foldernotes-is-a-parent-of-documentmeeting_notesdoc">04. Create a new relationship tuple to indicate that `folder:notes` is a `parent` of `document:meeting_notes.doc`</h3>
...
<h3 id="05-check-if-bob-is-an-editor-of-documentmeeting_notesdoc">05. Check if `bob` is an `editor` of `document:meeting_notes.doc`</h3>
...
For more information on this topic, see [Referencing Relations on Related Objects](/docs/configuration-language#referencing-relations-on-related-objects).
</Warning>
...
<CardGroup>
  <Card title="Modeling Concepts: Object to Object Relationships" href="/docs/modeling/building-blocks/object-to-object-relationships">
