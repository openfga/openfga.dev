...
Roles and permissions can be modeled within OpenFGA using an [authorization model](/docs/concepts#what-is-an-authorization-model) and [relationship tuples](/docs/concepts#what-is-a-relationship-tuple).

- **Roles** are assigned to [users](/docs/concepts#what-is-a-user) or a group of users. Any user can have more than one role, like `editor` or `owner`.
- **Permissions** allow users to access certain [objects](/docs/concepts#what-is-an-object) based on their specific roles, like `device_renamer` or `channel_archiver`.
...
Familiarize yourself with the basics of [OpenFGA concepts](/docs/concepts).

Assume that you have the following [authorization model](/docs/concepts#what-is-an-authorization-model) and a [type](/docs/concepts#what-is-a-type) called `trip` that users can be related to as `viewer` and/or an `owner`.
...
- A **[Relationship Tuple](/docs/concepts#what-is-a-relationship-tuple)**: a group stored in OpenFGA that consists of a user, a relation, and an object
- A **[Relationship](/docs/concepts#what-is-a-relationship)**: OpenFGA will be called to check if there is a relationship between a user and an object, indicating that the access is allowed
...
- A **[Check API Request](/docs/concepts#what-is-a-check-request)**: used to check for relationships between users and objects
</Accordion>
...
3. [Check](/docs/concepts#what-is-a-check-request) user roles and their permissions based on relationship tuples for direct and implied relations
...
Your type definitions reflects the roles and permissions on how bookings can be viewed/added, so you can create [relationship tuples](/docs/concepts#what-is-a-relationship-tuple) to assign roles to users, then [check](/docs/concepts#what-is-a-check-request) if users have the proper permissions.
