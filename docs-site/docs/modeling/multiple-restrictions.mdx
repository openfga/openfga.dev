...
With the above authorization model and relationship tuples, OpenFGA will correctly respond with `{"allowed":true}` when *[check](/docs/concepts#what-is-a-check-request)* is called to see if Carl and Becky can write this `document`.
