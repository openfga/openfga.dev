...
sidebarTitle: "Direct relationships"
...
Disabling _direct relationships_ for a certain relation on an object is useful especially in cases where you are trying to model some permissions that are not usually granted individually to a user.
...
To better understand this guide, you should be familiar with some [OpenFGA concepts](/docs/concepts) and know how to develop the things listed below.
...
<Accordion title="Before you start" id="before-you-start-details">
...
- [Direct Relationship Type Restrictions](/docs/configuration-language#direct-relationship-type-restrictions): used in the context of the relation definition can be used to allow direct relationships to the objects of this type
</Accordion>

## What are direct relationships?

Direct relationships are relationships where a user has a relationship to an **[object](/docs/concepts#what-is-an-object)** that is not dependent on any other relationship they have with that object.

When checking for a relationship, a direct relationship exists if a _**[relationship tuple](/docs/concepts#what-is-a-relationship-tuple)**_ is present in the system with the exact same object and relation that were in the query and where the user is one of:
...
Direct relationships can be enabled for a specific relation on an _**[object type](/docs/concepts#what-is-a-type)**_ by adding [direct relationship type restrictions](/docs/configuration-language#direct-relationship-type-restrictions) from that **[relation's definition](/docs/concepts#what-is-a-relation-definition)**. Likewise, they can be disabled by removing the direct relationship type restrictions.
...
<Note>
The **[authorization model](/docs/concepts#what-is-an-authorization-model)** describes two **[object types](/docs/concepts#what-is-a-type)**: `document` and `team`.

The `document` **[type definition](/docs/concepts#what-is-a-type-definition)** has two **[relations](/docs/concepts#what-is-a-relation)**, `editor` and `viewer`. Both relations allow a direct relationship; `viewer` also allows an **[indirect relationship](/docs/concepts#what-are-direct-and-implied-relationships)** through `editor`.
...
Now if we do a **[check request](/docs/concepts#what-is-a-check-request)** to see if Anne can view the planning document, we will get a response of `{"allowed": true}`.
...
<Tab title="Type Bound Public Access">

Assume you have a **[type bound public access](/docs/concepts#what-is-type-bound-public-access)** tuple where everyone of type `user` is a `viewer` of `document:planning` (In other words, the document is public)
...
[_Usersets_](/docs/modeling/building-blocks/usersets) are the third way direct relationships apply, we will see how in this section.
...
<Tab title="Indirect Relationship">

Here we will cover one example of an **[indirect relationship](/docs/concepts#what-are-direct-and-implied-relationships)** in order to see how they differ from direct relationships.
...
Check the following sections for more on how direct relationships can be used. Also, take a look at the access relation in the feature type in Modeling Entitlements for another use-case.

<CardGroup>
  <Card title="Modeling Roles and Permissions" href="/docs/modeling/roles-and-permissions">
    Learn how to remove the direct relationship to indicate nonassignable permissions.
  </Card>
  <Card title="Modeling for IoT" href="/docs/modeling/advanced/iot">
    See how Roles and Permissions can be used in an IoT use-case.
  </Card>
  <Card title="Modeling Entitlements" href="/docs/modeling/advanced/entitlements">
