...
sidebarTitle: "Concentric relationships"
...
In this short guide, you'll learn how to represent a concentric [relationships](/docs/concepts#what-is-a-relationship).
...
You will start with the _[authorization model](/docs/concepts#what-is-an-authorization-model)_ below, it represents a `document` _[type](/docs/concepts#what-is-a-type)_ that can have users **[related](/docs/concepts#what-is-a-relation)** as `editor` and `viewer`.
...
With the current type definition, there isn't a way to indicate that all `editors` of a certain `document` are also automatically `viewers` of that document. So for a certain user, in order to indicate that they can both `edit` and `view` a certain `document`, two _[relationship tuples](/docs/concepts#what-is-a-relationship-tuple)_ need to be created (one for `editor`, and another for `viewer`).
