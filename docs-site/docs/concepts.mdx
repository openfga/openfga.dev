...
The [OpenFGA](/docs/fga) service answers [authorization](/docs/authorization-concepts#authentication-and-authorization) [checks](#what-is-a-check-request) by determining whether a [relationship](#what-is-a-relationship) exists between an [object](#what-is-an-object) and a [user](#what-is-a-user). Checks reference your [authorization model](#what-is-an-authorization-model) against your [relationship tuples](#what-is-a-relationship-tuple) for authorization authority. Below are explanations of basic FGA concepts, like [type](#what-is-a-type) and [authorization model](#what-is-an-authorization-model), and a [playground](https://play.fga.dev/) to test your knowledge.
...
<Accordion title="Examples and details" id="what-is-a-type-definition-examples">
...
<Accordion title="Examples and details" id="what-is-an-authorization-model-examples">
...
<Accordion title="Examples and details" id="what-is-a-store-examples">
...
<Accordion title="Examples and details" id="what-is-an-object-examples">
...
<Accordion title="Examples and details" id="what-is-a-user-examples">
...
<Accordion title="Examples and details" id="what-is-a-relation-examples">
...
<Accordion title="Examples and details" id="what-is-a-relation-definition-examples">
  For example:
...
<Accordion title="Examples and details" id="what-is-a-directly-related-user-type-examples">
...
<Accordion title="Examples and details" id="what-is-a-condition-examples">
  ...
  <OpenFGACodeBlock code={`condition less_than_hundred(x: int) {
\x20 x < 100
}`} />
</Accordion>
...
A **relationship tuple** is a base tuple/triplet consisting of a user, relation, and object. Tuples may add an optional condition, like [Conditional Relationship Tuples](#what-is-a-conditional-relationship-tuple). Relationship tuples are written and stored in OpenFGA.

<Accordion title="Examples and details" id="what-is-a-relationship-tuple-examples">
...
<Accordion title="Examples and details" id="what-is-a-conditional-relationship-tuple-examples">
...
<Accordion title="Examples and details" id="what-is-a-relationship-examples">
...
A **direct relationship** (R) between user X and object Y means the relationship tuple (user=X, relation=R, object=Y) exists, and the OpenFGA authorization model for that relation allows the direct relationship because of [direct relationship type restrictions](/docs/configuration-language#direct-relationship-type-restrictions).
...
<Accordion title="Examples and details" id="what-are-direct-and-implied-relationships-examples">
...
<Accordion title="Examples and details" id="what-is-a-check-request-examples">
  Check requests use the `check` methods in the OpenFGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [check endpoint](/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object) using curl or in your code. The check endpoint responds with `{ "allowed": true }` if a relationship exists, and with `{ "allowed": false }` if the relationship does not.

  For example, the following will check whether `anne` of type user has a `viewer` relation to `document:new-roadmap`:

  <CheckRequestViewer user={'user:anne'} relation={'viewer'} object={'document:new-roadmap'} allowed={true} />

  For more information, see the [Relationship Queries page](/docs/interacting/relationship-queries) and the official [Check API Reference](/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object).
</Accordion>
...
<Accordion title="Examples and details" id="what-is-a-list-objects-request-examples">
  List objects requests are completed using the `listobjects` methods in the OpenFGA SDKs ([JavaScript SDK](https://www.npmjs.com/package/@openfga/sdk)/[Go SDK](https://github.com/openfga/go-sdk)/[.NET SDK](https://www.nuget.org/packages/OpenFga.Sdk)) by manually calling the [list objects endpoint](/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with) using curl or in your code.
  ...
  For more information, see the [Relationship Queries page](/docs/interacting/relationship-queries) and the [List Objects API Reference](/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with).
</Accordion>
...
<Accordion title="Examples and details" id="what-is-a-list-users-request-examples">
  List users requests are completed using the relevant `ListUsers` method in SDKs, the `fga query list-users` command in the CLI, or by manually calling the [ListUsers endpoint](/api-reference/relationship-queries/list-the-users-matching-the-provided-filter-who-have-a-certain-relation-to-a-particular-type) using curl or in your code.
  ...
  For more information, see the [ListUsers API Reference](/api-reference/relationship-queries/list-the-users-matching-the-provided-filter-who-have-a-certain-relation-to-a-particular-type).
</Accordion>
...
<Accordion title="Examples and details" id="what-are-contextual-tuples-examples">
  ...
  For more information, see [Contextual and Time-Based Authorization](/docs/modeling/contextual-time-based-authorization), [Authorization Through Organization Context](/docs/modeling/organization-context-authorization) and [Check API Request Documentation](/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object).
</Accordion>
...
<Accordion title="Examples and details" id="what-is-type-bound-public-access-examples">
