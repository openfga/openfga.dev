...
sidebarTitle: "Search with Permissions"
---
...
1. Call [`/batch-check`](/docs/interacting/relationship-queries#batch-check) to check access for multiple objects in a single request.
...
1. Call the [OpenFGA changes API](/api-reference/relationship-tuples/return-a-list-of-all-the-tuple-changes).
...
1. Call the [OpenFGA List Objects API](/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with). to get the list of all resources a user can access.
