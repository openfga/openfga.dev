...
OpenFGA is a [ReBAC](https://openfga.dev/docs/learn/rebac.md) engine that **also models RBAC cleanly**: a role is a relation on a tenant, and the model lets the same user be a different role in different tenants without any duplication. The [Modeling Roles](https://openfga.dev/docs/best-practices/modeling-roles.md) guide walks through the pattern.
