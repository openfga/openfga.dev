...
A "store" is a <ProductName format={ProductNameFormat.LongForm}/> entity used for organizing data.  

</summary>

Each store contains a single [authorization model](#what-is-an-authorization-model) and may contain various [relationship tuples](#what-is-a-relationship-tuple). You may wish to create a second store if you need to have a second model. 
