...
:::info Note
The Consistency parameter is an experimental feature in OpenFGA and needs to be enabled with the `enable-consistency-params` parameter, e.g. `openfga run --experimentals enable-consistency-params`.
:::

## Background

When querying <ProductName format={ProductNameFormat.ShortForm}/> using any of the query APIs like [check, expand, read, list-objects and list-users requests](./relationship-queries.mdx), you can specify a query consistency parameter that can have one of the following values:
...
When specifying `HIGHER_CONSISTENCY` you are trading off consistency for latency. Always specifying `HIGHER_CONSISTENCY` will have a significant impact in performance.
...
```javascript
    if (date_modified + cache_time_to_live_period > Date.now()) { 
        fgaClient.check(
            { "user" : "user:anne", "relation" : "user:can_view", "object" : "document:readme" }
         )
    } else {
        fgaClient.check(
            { "user" : "user:anne", "relation" : "user:can_view", "object" : "document:readme" }, consistency = HIGHER_CONSISTENCY
         )
    }
```
...
<ProductName format={ProductNameFormat.ShortForm}/> will have a similar feature in future releases.
...
<RelatedSection
  ...
  relatedLinks={[
    {
      title: 'Check, Read and Expand',
      description: 'Comparison Between Check, Read And Expand API Calls.',
      link: './check-read-expand',
      id: './check-read-expand',
    }
  ]}
/>
