...
Creating an authorization model and a relationship tuple grants a user access to an object. To learn more, [read about Direct Access.](./direct-access.mdx)

### Modeling public access

The following example uses public access. To learn more, [read about Public Access.](./direct-access.mdx)
...
The Write API accepts up to 40 unique tuples in the request, so you can use one API call that converts the `tweet` from public to visible to only the `user`'s followers.

Note: the API limit above is the limit of tuples per write transaction and applies to the sum of both writes and deletes in that request. For more information abour rate limits, see [the subscription plan page](../subscription-plans.mdx).
