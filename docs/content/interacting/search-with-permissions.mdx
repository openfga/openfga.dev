...
- Get a list of IDs for a resource higher in the hierarchy that has lower cardinality. For example, in a document-management application, you can get the list of folders the user can access, and then in the database query, filter the documents based on that list of folders . The chances the user will be able to see most of the documents in those folders should be be high.
