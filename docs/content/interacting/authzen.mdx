...
`options.evaluations_semantic` only accepts defined values (`execute_all`, `deny_on_first_deny`, `permit_on_first_permit`). Unknown values are rejected with `InvalidArgument`.
