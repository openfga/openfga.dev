...
## Concurrency Limits
Check and ListObjects queries can potentially involve a high-degree of depth or breadth to resolve the query. If a query involves a lot of recursion or indirect group lookups, then it's possible that a single query could dominate the server's resources and the database connections. For this reason, we recommend setting some concurrency limits to protect OpenFGA servers.
...
Developers can configure the server settings above to limit the amount of concurrent evaluations/reads that can be in effect per request. These settings will effectively queue/throttle evaluation paths so as to reduce the total concurrency from a single query. The default values for these are essentially unbounded (e.g. `MaxUint32`).
