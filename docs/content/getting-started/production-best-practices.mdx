...
It's strongly recommended to fine-tune your server database connection settings to avoid having to re-establish database connections frequently. Establishing database connections is slow and will negatively impact performance, and so here are some guidlines for managing database connection settings:
...
* If a single ListObjects query is negatively impacting other query endpoints, then consider setting a stricter value for `OPENFGA_MAX_CONCURRENT_READS_FOR_LIST_OBJECTS`.

* If a single Check query is negatively impacting other query endpoints, then consider setting a stricter value for `OPENFGA_MAX_CONCURRENT_READS_FOR_CHECK`.
