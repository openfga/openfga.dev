...
:::note
The BatchCheck endpoint is currently only supported by the JS SDK (>=[v0.8.0](https://github.com/openfga/js-sdk/releases/tag/v0.8.0) and the Python SDK (>=[v0.8.0](https://github.com/openfga/python-sdk/releases/tag/v0.9.0)). Support in the other SDKs is being worked on.
...
The BatchCheck endpoint currently accepts a maximum of 50 checks per request (configurable on the server).
...
* if you want to pass in more checks than the server supports in a single call, you may instruct the SDK to batch the `BatchCheck` requests for you
:::
