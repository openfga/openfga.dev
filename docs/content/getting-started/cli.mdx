...
- Storing configuration values in a .fga.yaml located in one of the following directories, in order of precedence (top has highest priority):
  - The current working directory
  - The user-specific config directory: 
    - On unix systems: $XDG_CONFIG_HOME value if provided, otherwise $HOME/.config
    - On windows: %AppData%
  - The fga subdirectory under the user-specific config directory
  - The current user's home directory:
    - On unix systems: $HOME
