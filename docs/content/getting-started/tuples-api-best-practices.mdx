...
- Avoid using intersection or exclusion in ListObjects authorization model
...
2. In some cases, tuple information flows through Data Sub Processors such as ones used for logging and tracing. You’ll minimize risk if you don’t include sensitive data in tuples.

## Always Specify Authorization Model ID Whenever Possible

It is strong recommended that authorization model ID be specified in your Relationship Queries (such as [Check](./perform-check.mdx) and [ListObjects](../interacting/relationship-queries.mdx#listobjects)) and Relationship Tuples (such as [Read](../interacting/relationship-queries.mdx#read) and [Write](./update-tuples.mdx)) request.
