...
Tracing by default uses a secure connection. You can disable it using `--trace-otlp-insecure` flag or the standard otlp environment variable `OTEL_EXPORTER_OTLP_TRACES_INSECURE`.
