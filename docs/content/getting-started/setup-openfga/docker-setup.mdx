...
3. Run `docker run -p 8080:8080 -p 8081:8081 openfga/openfga run`.

This will start an HTTP server and gRPC server with the default configuration options. Port 8080 is used to serve the HTTP API and 8081 is used to serve the gRPC API.

The [Playground](./playground.mdx) is disabled by default. To serve it from a container, enable it and bind it to all interfaces, then publish port 3000:
...
:::caution Warning
Without `--playground-addr`, the Playground listens on `127.0.0.1:3000` inside the container, which a published port cannot reach. Do not enable the Playground in production.
:::

It is then available at http://localhost:3000/playground.
...
services:
  ...
  openfga:
    ...
    environment:
      ...
      # The playground binds to 127.0.0.1 unless an address is given,
      # which the published port below cannot reach
...
```shell
docker run --name openfga --network=openfga -p 8080:8080 -p 8081:8081 openfga/openfga run \
