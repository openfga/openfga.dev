...
slug: /getting-started/setup-openfga/reporting-performance-issues
sidebar_position: 6
---
...
If you're experiencing performance issues with OpenFGA, gathering the right information upfront will help us diagnose the problem. The more details you can share, the faster we can help. 

This guide outlines the key details necessary to investigate performance-related issues.
...
3. **Script or steps** to reproduce the performance problem
...
Build a simplified model and generate mock tuples that demonstrate the same performance characteristics. This approach still allows us to reproduce the issue without exposing sensitive data.
:::
...
1. When did the performance issue start?
...
3. Is the performance issue consistent or intermittent? If intermittent, how often does it occur?
...
If you can't share your real tuples due to data sensitivity, consider creating a **mock dataset** that reproduces the same performance issue. This allows us to reproduce the problem without handling sensitive information. Generate fake tuples with similar:
...
:::tip
The absolute best way to help us diagnose your performance issue is to provide a **full reproduction** (model + tuples + script). If that's not possible, providing as much of the information above as you can will still help us investigate and resolve your performance issue.
:::
