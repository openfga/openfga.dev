...
In OpenFGA [v1.7.0](https://github.com/openfga/openfga/releases/tag/v1.7.0), we introduced an experimental built-in access control feature that allows you to control access to your OpenFGA server.

Currently, there is no provided way to initialize the store and model for access control, nor is there a way to bootstrap the client IDs that are supposed to be admins.

:::caution Warning
The built-in access control feature in OpenFGA is experimental and is not recommended for production use. We are looking for feedback on this, so if you do try it, please reach out on our [openfga Slack channel](../../community) in the CNCF community.
:::

Following are the steps on how to enable access control.

## Requirements

- OIDC Provider: You need to have an OIDC provider [set up and configured](./configure-openfga#oidc) in your OpenFGA server. OpenFGA currently requires an OIDC provider to be set up to use access control.
...
- The FGA CLI: While the CLI is not strictly required, you need to follow the steps below. You can install it by following the instructions [here](../cli). If you do not want to use the CLI, you can call the API with the equivalent SDK or REST calls.

## Ensure the server is running (with access control disabled)
...
## Create the access control store and model

We will be using the following model to enable access control.

:::note Customizing your access control model
You may choose to modify this model to suit your needs, however please keep in mind we may not allow configuring the model in the future and you will need to do your own migrations in that case.
...
2. Run the following command to create the store and model:
   ...
   ```
   It will print out a store ID and model ID. You will need these IDs in the following steps.
3. Grant your initial client ID access
   You can do so by writing a tuple to the access control store you just created. The tuple should be of the type `application` and should have the `client_id` field set to the client ID of the client you want to grant access to. You can use the FGA CLI to do this:
   ```shell
...
### Enable access control in the server
...
### Customize what the claim you want the API to use (optional)

By default, the API will use the following claims (in order) the OIDC token to identify the client. If you want to use a different claim, you can set the environment variable `OPENFGA_AUTHN_OIDC_CLIENT_ID_CLAIMS` to the claim(s) you want to use.

If the claims are not set in the configuration, the following claims as default (in order):
...
2. Grant access to writing tuples of a certain module in a store
   In order to grant access only  the module feature, you must have a model with modules. Read the [modular models documentation](../../modeling/modular-models) for more.
