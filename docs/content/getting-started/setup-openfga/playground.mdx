...
It is disabled by default. Enable it with `--playground-enabled`, and it is served on port 3000, accessible at http://localhost:3000/playground.

When running OpenFGA in a container, also pass `--playground-addr 0.0.0.0:3000`. The Playground otherwise listens on `127.0.0.1:3000` inside the container, which a published port cannot reach. See the [Docker setup guide](./docker-setup.mdx).
