...
| OPENFGA_CORS_ALLOWED_ORIGINS              | If specify, HTTP server will only allowed specific origins for cross origin resource sharing.                                                                                                                                                    | No       | array of string          | *             | `http://localhost,http://api.openfga.example`      |
| OPENFGA_CORS_ALLOWED_HEADERS              | If specify, HTTP server will only allowed specific headers during preflight CORS option check.                                                                                                                                                   | No       | array of string          | *             | `Origin,Accept,Content-Type,X-Requested-With`      |
