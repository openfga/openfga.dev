...
Various releases of OpenFGA may have experimental features that can be enabled by providing the `--enable-experimentals` flag or the `enableExperimentals` config.

```
docker run -p 8080:8080 -p 8081:8081 openfga/openfga run --enable-experimental="feature1, feature2"
```
...
|------------------------|----------------------------------------------------------------------------------------------------------|-----------------|
| list-objects-optimized | Enables support for an optimization of the [openfga.v1.ListObjects](/api/service#/Relationship%20Queries/ListObjects) API for models with schema version 1.1 | >= v0.3.1       |

:::caution Warning
Experimental features are not guaranteed to be stable and may lead to server instabilities. It is not recommended to enable
experimental features for anything other than experimentation.

Experimental feature flags are also not considered part of API compatibility and are subject to change, so please refer to each 
OpenFGA specific release for a list of the experimental feature flags that can be enabled for that release.
:::
