...
You may configure the OpenFGA sever with `config.yaml` file. The config.yaml is be specified in either 
- `/etc/openfga`
- `$HOME/.openfga`
...
An example of configuration to use postgres as database and configured with preshared authentication keys "key1, key2" is:
...
- Update the configuration as follows:
  ...
  - Enable production logging by changing the log format configuration to `json`.
