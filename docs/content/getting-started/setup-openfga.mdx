...
| OPENFGA_HTTP_TLS_ENABLED | Whether or not to enable TLS for the gRPC server. If `true` then you must also set `OPENFGA_HTTP_TLS_CERT_PATH` and `OPENFGA_HTTP_TLS_KEY_PATH`. | Yes | boolean | false | false
