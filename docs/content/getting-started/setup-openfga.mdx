...
OpenFGA metrics are collected with the [Prometheus data format](https://prometheus.io/docs/concepts/data_model/) and exposed on address `http://localhost:2112/metrics`.

They are collected and exposed by default, but you can disable this by setting the flag `--metrics-enabled=false` (or `OPENFGA_METRICS_ENABLED=false` environment variable).
...
Collection and exporting is disabled by default, but you can enable both with the `--trace-enabled=true` flag (`OPENFGA_TRACE_ENABLED=true` environment variable). Traces will be exported by default to address  `http://localhost:4317`. You can change this address with the `--trace-otlp-endpoint` flag (`OPENFGA_TRACE_OTLP_ENDPOINT` environment variable).
