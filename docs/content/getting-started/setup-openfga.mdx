...
3. Run `docker run -p 8080:8080 -p 3000:3000 openfga/openfga run`.

This will start an HTTP server and gRPC server with the default configuration options. Port 8080 will be used for HTTP API request and port 3000 will be used for hosting [playground](#disabling-playground).
...
By default, the playground will be enabled for OpenFGA and port 3000 will be used as default.  To disable the playground, you will need to [configure the server](#configuring-the-server).
