---
sidebar_position: 3
slug: /writing-data/managing-user-access
---
...
With the above, we have added a [**direct** relationship](../modeling/concepts/direct-relationships.mdx) between Anne and `tweet:1`. When we call the Check API to see if Anne has a `reader` relationship, <ProductName format={ProductNameFormat.ShortForm}/> will say yes.
...
With this, we have removed the [direct relationship](../modeling/concepts/direct-relationships.mdx) between Anne and `tweet:1`. And because our type definition for `reader` does not include any other relations, a call to the Check API will now return a negative response.
