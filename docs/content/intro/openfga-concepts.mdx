---
title: OpenFGA Concepts
sidebar_position: 2
slug: /intro/openfga-concepts
---
...
# <ProductName format={ProductNameFormat.LongForm}/> Concepts

<DocumentationNotice />

The <ProductName format={ProductNameFormat.ProductLink}/> service answers <IntroductionSection linkName="authorization" section="what-is-authorization"/> [checks](#what-is-a-check-request) by determining whether a **[relationship](#what-is-a-relation)** exists between an [object](#what-is-an-object) and a [user](#what-is-a-user). Checks takes into consideration the **[authorization model](#what-is-an-authorization-model)** of the system and the **[relationship tuples](#what-is-a-relationship-tuple)** present in the system at that time in order to make a decision.
...
- A JSON syntax accepted by the <ProductName format={ProductNameFormat.ShortForm}/> API that follows closely the original syntax discussed in the [Zanzibar Paper](https://research.google/pubs/pub48190/). See [Equivalent Zanzibar Concepts](../modeling/configuration-language.mdx#equivalent-zanzibar-concepts).
...
Learn more about the <UpdateProductNameInLinks link="../modeling/configuration-language" name="{ProductName} Configuration Language" />.
...
A **store** is a <ProductName format={ProductNameFormat.LongForm}/> organization entity that owns an authorization model.

</summary>

A store contains all your data, including the [authorization model](#what-is-an-authorization-model) and [relationship tuples](#what-is-a-relationship-tuple).
...
For more information, please see [Modeling Basics](../modeling/basics/modeling-basics.mdx).
...
- `anne` has a **direct relationship** with `document:new-roadmap` as `viewer` if the [type definition](#what-is-a-type-definition) allows it (allows [`self`](../modeling/configuration-language.mdx#the-direct-relationship-keyword)), and one of the following [relationship tuples](#what-is-a-relationship-tuple) exist:
...
Unlike relationship tuples, they are not written to the system state. However, if contextual tuples are sent alongside a check request, in the context of this particular check request, they are treated as have been written to the system state.
...
Note that `*` has no special meaning when used in other contexts. For example, `workspace:*` as an [object](#what-is-an-object) means a single object with the type `workspace` and identifier the string `*`.
For more information, please see [Modeling Public Access](../modeling/basics/public-access.mdx) and [Advanced Modeling: Modeling Google Drive](../modeling/advanced/use-cases/gdrive.mdx).
  ...
  relatedLinks={[
    {
      ...
      id: '../modeling/configuration-language',
    },
    {
      ...
      id: '../modeling/basics/modeling-basics',
    },
  ]}
/>
