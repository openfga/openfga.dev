...
:::caution
The original 1.0 schema is considered deprecated, and support for it is scheduled to be dropped. Please see the [deprecation timeline](#deprecation-timeline).
:::
...
2- **March 15th 2023**: Disallow writing and evaluating new 1.0 models with OpenFGA version `0.4.0`
  - The API will no longer accept writing new 1.0 models (affects [`WriteAuthorizationModel`](/api/service#/Authorization%20Models/WriteAuthorizationModel), [`WriteAssertions`](/api/service#/Assertions/WriteAssertions)) unless the `OPENFGA_ALLOW_WRITING_1_0_MODELS` flag is used to override the setting. Note: reading those earlier models and their assertions will keep working.
  - Evaluation requests ([`Check`](/api/service#/Relationship%20Queries/Check), [`Expand`](/api/service#/Relationship%20Queries/Expand), [`ListObjects`](/api/service#/Relationship%20Queries/List%20Objects)) and [`Write`](/api/service#Relationship%20Tuples/Write) against 1.0 models will, by default, no longer be accepted unless the `OPENFGA_ALLOW_EVALUATING_1_0_MODELS` flag is used to override the setting.
