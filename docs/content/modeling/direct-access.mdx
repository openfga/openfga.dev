...
Below, you'll add a _<ProductConcept section="what-is-a-relationship-tuple" linkName="relationship tuple" />_ to indicate that `bob` is an `editor` of `document:meeting_notes.doc` by adding the following:
