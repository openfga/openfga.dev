...
- Are only supported on the <UpdateProductNameInLinks link="https://openfga.dev/api-reference/relationship-queries/check-whether-a-user-is-authorized-to-access-an-object" name="Check API endpoint" /> and <UpdateProductNameInLinks link="https://openfga.dev/api-reference/relationship-queries/list-all-objects-of-the-given-type-that-the-user-has-a-relation-with" name="ListObjects API endpoint" />. They are not supported on read, expand and other endpoints.
- If you are using the <UpdateProductNameInLinks link="https://openfga.dev/api-reference/relationship-tuples/return-a-list-of-all-the-tuple-changes" name="Read Changes API endpoint" /> to build a permission aware search index, note that it will not be trivial to take contextual tuples into account.
