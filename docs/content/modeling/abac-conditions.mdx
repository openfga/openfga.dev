...
:::note
The type restriction for `document#viewer` requires that any `user` subject that is written in the relationship tuple must be accompanied by the `temporal_grant` condition. This is denoted by the `user with temporal_grant` specification.
:::
