...
This guide shows how to model tool-level authorization for an MCP server using <ProductName format={ProductNameFormat.ShortForm}/>, covering public tools, role-based access, group membership, temporal access, and resource-level permissions within tools. We are authorizing how can a agent interact with an MCP server based on a model based on user relationships. 
