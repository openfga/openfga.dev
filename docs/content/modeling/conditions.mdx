...
## Overview
**Conditions** allow more complex authorization modeling scenarios involving attributes and can be used to represent some Attribute-based Access Control (ABAC) policies. Take a look at the [Conditions](../concepts.mdx#what-is-a-condition) and [Conditional Relationship Tuples](../concepts.mdx#what-is-a-conditional-relationship-tuple).
...
For background information, see our blog post: [Conditional Relationship Tuples for OpenFGA](https://openfga.dev/blog/conditional-tuples-announcement). 

## Defining Conditions in Models
The following authorization model to demonstrates a temporal-based access policy in which a given user can view a document if (and only if) they have been granted the viewer relationship and their grant policy not expired.
...
:::note
The type restriction for `document#viewer` requires that any user of type `user` in the relationship tuple must be accompanied by the `non_expired_grant` condition. This is denoted by `user with non_expired_grant`.
:::
...
When you [Write](https://openfga.dev/api/service#/Relationship%20Tuples/Write) relationship tuples to the OpenFGA store, as in the model above, any `document#viewer` relationship with `user` objects must be accompanied by the condition `non_expired_grant` because the type restriction requires it.

For example,  `user:anne` can be given viewer access to `document:1` for 10 minutes by writing the following relationship tuple:
