...
You'll encounter the JSON syntax when calling the API directly or through the [SDKs](../integration), and the DSL when interacting with <ProductName format={ProductNameFormat.ShortForm}/> through the Playground. Throughout the documentation you can switch between the two presentations.
