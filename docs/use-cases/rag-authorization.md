...
3. **OpenFGA filters** the candidates: for each candidate `doc:X`, check whether `user:Y` has `can_view`. Or call [list-objects](https://openfga.dev/docs/interacting/relationship-queries.md) once to get the full set of documents this user can read, then intersect.
...
If access depends on document attributes (classification, region, time window) as well as relationships, use [conditions](https://openfga.dev/docs/modeling/conditions.md) and [contextual tuples](https://openfga.dev/docs/interacting/contextual-tuples.md). Both are evaluated at check time without round-tripping back to your application.

## Related reading

- [RAG authorization modeling guide](https://openfga.dev/docs/modeling/agents/rag-authorization.md)
- [AI agent authorization](https://openfga.dev/docs/use-cases/ai-agent-authorization.md)
- [List-objects API](https://openfga.dev/docs/interacting/relationship-queries.md)
