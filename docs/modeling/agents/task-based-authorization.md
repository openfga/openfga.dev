...
When checking whether `task:2` can call `tool_resource:slack_send_message/XGA14FG`, send a [contextual tuple](https://openfga.dev/docs/interacting/contextual-tuples.md) linking the resource to its tool. This avoids storing a tuple for every channel — you provide the tool-to-resource relationship at query time.
...
You can use OpenFGA [conditions](https://openfga.dev/docs/configuration-language.md#conditional-relationships) to make permissions expire after a duration, or limit how many times the tool can be called.
...
The examples above do not verify that the agent making the call is actually assigned to the task. You can enforce this using [contextual tuples](https://openfga.dev/docs/interacting/contextual-tuples.md) and an intersection (`and`) in the model, similar to the [Authorization Through Organization Context](https://openfga.dev/docs/modeling/organization-context-authorization.md) pattern.
