...
In this short guide, you'll learn how to represent a concentric [relationships](https://openfga.dev/docs/concepts.md#what-is-a-relationship).
...
With the current type definition, there isn't a way to indicate that all `editors` of a certain `document` are also automatically `viewers` of that document. So for a certain user, in order to indicate that they can both `edit` and `view` a certain `document`, two _[relationship tuples](https://openfga.dev/docs/concepts.md#what-is-a-relationship-tuple)_ need to be created (one for `editor`, and another for `viewer`).
