...
OpenFGA expresses the limit _as part of the relationship_ using [conditions](https://openfga.dev/docs/modeling/conditions.md), so the check `can_initiate_transfer` takes the transaction amount as a contextual parameter and returns a single allow/deny.

## Where this maps to OpenFGA features

| Banking requirement               | OpenFGA feature                                                                                       |
| --------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Per-actor transaction limits      | [conditions](https://openfga.dev/docs/modeling/conditions.md) on `can_initiate_transfer`              |
...
| Time-bounded delegation           | [contextual tuples](https://openfga.dev/docs/interacting/contextual-tuples.md) carrying expiry        |
| Tenant isolation per bank brand   | tenant-scoped types, see [multi-tenant SaaS](https://openfga.dev/docs/use-cases/multi-tenant-saas.md) |
| Audit who-could-have-done-what    | [Read Changes API](https://openfga.dev/docs/interacting/read-tuple-changes.md)                        |
...
The schema, sample tuples, and assertions are in [openfga/sample-stores/stores/banking](https://github.com/openfga/sample-stores/tree/main/stores/banking). For the broader pattern of "permissions plus runtime context", see [Modeling ABAC](https://openfga.dev/docs/best-practices/modeling-abac.md) and [Contextual and Time-Based Authorization](https://openfga.dev/docs/modeling/contextual-time-based-authorization.md).
