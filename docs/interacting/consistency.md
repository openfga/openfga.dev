...
When querying OpenFGA using Read or any of the query APIs like [Check, Expand, ListObjects and ListUsers](https://openfga.dev/docs/interacting/relationship-queries.md), you can specify a query consistency parameter that can have one of the following values:
...
Learn how to [configure OpenFGA](https://openfga.dev/docs/getting-started/setup-openfga/configure-openfga.md).
...
The [Zanzibar paper](https://openfga.dev/docs/authorization-concepts.md#what-is-zanzibar) has a feature called `Zookies`, which is a consistency token that is returned from Write operation. You can store that token in you resource table, and specify it in subsequent calls to query APIs.
