...
\x20   define can_create_project: admin
\x20   define can_edit_billing_details: admin`} />
...
\x20 relations
\x20   define admin: [user]
\x20   define project_admin: [user]  # new role

\x20   # existing permissions
\x20   define can_edit_billing_details: admin\x20
\x20   define can_create_project: admin or project_admin

\x20   # new permissions for project admins
\x20   define can_edit_project: admin or project_admin
\x20   define can_view_project: admin or project_admin`} />
...
type organization
\x20 relations
\x20   define admin: [user]  # static role

\x20   # permissions can be assigned to custom roles or static roles
\x20   define can_create_project: [role#assignee] or admin\x20
\x20   define can_edit_project: [role#assignee] or admin\x20`} />
...
\x20   define can_create_project: [role#assignee] or admin\x20
\x20   define can_edit_project: [role#assignee] or admin\x20
\x20   define can_delete_project: [role#assignee] or admin  # new permission`} />
...
type role
\x20 relations
\x20   define can_view_project: [user:*]
\x20   define can_edit_project: [user:*]`} />
...
<OpenFGACodeBlock code={`type role_assignment
\x20 relations
\x20   define assignee: [user]
\x20   define role: [role]

\x20   define can_view_project: assignee and can_view_project from role
\x20   define can_edit_project: assignee and can_edit_project from role`} />
...
<OpenFGACodeBlock code={`type organization\x20
\x20 relations
\x20   define admin: [user]
...
\x20   define role_assignment: [role_assignment]
\x20\x20\x20\x20
\x20   # combine role assignments and static roles
\x20   define can_edit_project: can_edit_project from role_assignment or admin from organization
\x20   define can_view_project: can_view_project from role_assignment or admin from organization`} />
...
4. **Functional Testing**: [Write tests](/docs/modeling/testing) to verify your model behaves as expected
...
<CardGroup>
  <Card title="Custom Roles Step by Step" href="/docs/modeling/custom-roles">
    Follow a detailed walkthrough of implementing custom roles.
  </Card>
  <Card title="Multi-tenant RBAC Example" href="https://github.com/openfga/sample-stores/blob/main/stores/multitenant-rbac">
    See a complete multi-tenant role-based access control implementation.
  </Card>
  <Card title="Role Assignments Example" href="https://github.com/openfga/sample-stores/tree/main/stores/role-assignments">
