---
sidebarTitle: "Slack"
title: "Modeling Authorization for Slack with OpenFGA"
...
- How to indicate relationships between a group of **[users](/docs/concepts#what-is-a-user)** and an **[object](/docs/concepts#what-is-an-object)**.
  Used here to indicate that all members of a slack workspace can write in a certain channel.
  See [Modeling User Groups](/docs/modeling/user-groups) for more.
- How to Model **concentric relationship** to have a certain [relation](/docs/concepts#what-is-a-relation) on an object imply another relation on the same object.
  Used here to indicate that legacy admins have all the permissions of the more granular channels admin.
  See [Modeling Concentric Relationships](/docs/modeling/building-blocks/concentric-relationships) for more.
- How to use [**the union operator**](/docs/configuration-language#the-union-operator) condition to indicate that a user might have a certain relation with an object if they match any of the criteria indicated.
</Note>
...
<Accordion title="OpenFGA concepts" id="-concepts">
...
<h4 id="modeling-concentric-relationships">Modeling concentric relationships</h4>

You need to know how to update the authorization model to allow having nested relations such as all writers are readers. [Learn more →](/docs/modeling/building-blocks/concentric-relationships)

<h4 id="concepts--configuration-language">Concepts & configuration language</h4>

- Some [OpenFGA concepts](/docs/concepts)
- [Configuration Language](/docs/configuration-language)
</Accordion>
...
<h2 id="modeling-workspaces--channels">Modeling workspaces & channels</h2>
...
OpenFGA is based on [Zanzibar](https://zanzibar.academy), a Relation Based Access Control system. This means it relies on [objects](/docs/concepts#what-is-an-object) and [user](/docs/concepts#what-is-a-user) [relations](/docs/concepts#what-is-a-relation) to perform authorization [checks](/docs/concepts#what-is-a-check-request).
...
Here is how you would express than in OpenFGA's [authorization model](/docs/concepts#what-is-an-authorization-model): You have a [type](/docs/concepts#what-is-a-type) called "workspace", and users can be related to it as a legacy_admin, channels_admin, member and guest
...
[Direct relationship type restrictions](/docs/configuration-language#direct-relationship-type-restrictions) indicate that a user can have a [direct relationship](/docs/concepts#what-are-direct-and-implied-relationships) with an object of the type the relation specifies.
</Note>
...
To represent permissions in OpenFGA we use [relations](/docs/concepts#what-is-a-relation). For workspace permissions we need to create the following [authorization model](/docs/concepts#what-is-an-authorization-model):
...
The OpenFGA service determines if a [user](/docs/concepts#what-is-a-user) has access to an [object](/docs/concepts#what-is-an-object) by [checking](/docs/concepts#what-is-a-check-request) if the user has a relation to that object. Let us examine one of those relations in detail:
...
This means that a user can be [directly related](/docs/concepts#what-are-direct-and-implied-relationships) as a member to an object of type "workspace"
</Note>
...
And verify by [asking](/docs/concepts#what-is-a-check-request) OpenFGA
...
To verify, we can issue [check requests](/docs/concepts#what-is-a-check-request) to verify it is working as expected.
...
As you saw before, running this query will return `amy is not a member of workspace:sandcastle`, which is correct based on the data you have given OpenFGA so far. But in reality, Amy, who is a `legacy_admin` already has an [implied](/docs/concepts#what-are-direct-and-implied-relationships) `channels_admin` and `member` relations. In fact anyone (other than a guest) is a `member` of the workspace.

To change this behavior, we will update our system with a [**concentric relationship**](/docs/modeling/building-blocks/concentric-relationships) model.

With the following updated [authorization model](/docs/concepts#what-is-a-type-definition), you are informing OpenFGA that any user who is related to a workspace as `legacy_admin`, is also related as a `channels_admin` and a `member`.
...
So far, you have modeled the users' [relations](/docs/concepts#what-is-a-relation) to the workspace itself. In this task you will expand the model to include the relations concerning the channels.
...
There is an [implied relation](/docs/concepts#what-are-direct-and-implied-relationships) that anyone who can write to a channel can also read from it, so the authorization model can be modified to be:
...
- the set of users with a [direct](/docs/concepts#what-are-direct-and-implied-relationships) viewer relation to this object
...
What remains is to add the [relationship tuples](/docs/concepts#what-is-a-relationship-tuple) to indicate the relation between the users, workspace and the channels.
...
##### <span id="general-channel" style={{ scrollMarginTop: '7rem' }}><code>#general</code> channel</span>
...
##### <span id="marketing-internal-channel" style={{ scrollMarginTop: '7rem' }}><code>#marketing-internal</code> channel</span>
...
##### <span id="proj-marketing-campaign-channel" style={{ scrollMarginTop: '7rem' }}><code>#proj-marketing-campaign</code> channel</span>
...
- Have a basic understanding of [authorization](/docs/authorization-concepts#authentication-and-authorization) and [OpenFGA Concepts](/docs/concepts).
- Understand how to model authorization for a communication platform like Slack using OpenFGA.

In this tutorial, you:

- were introduced to [fine-grained authorization](/docs/authorization-concepts#what-is-fine-grained-authorization) and OpenFGA.
