...
title: "Modeling Google Drive permissions with OpenFGA"
...
This tutorial explains how to represent [Google Drive](https://www.google.com/intl/en-GB/drive/) permissions model with OpenFGA.

<Note>
**What you will learn**

- Indicate [relationships](/docs/concepts#what-is-a-relationship) between a group of **[users](/docs/concepts#what-is-a-user)** and an **[object](/docs/concepts#what-is-an-object)**. See [Modeling User Groups](/docs/modeling/user-groups) for more.
  Used here to indicate that all users within a domain can access a document (sharing a document within an organization).
- Model **concentric relationship** to have a certain [relation](/docs/concepts#what-is-a-relation) on an object imply another relation on the same object. See [Modeling Concepts: Concentric Relationships](/docs/modeling/building-blocks/concentric-relationships) for more.
  Used here is to indicate that writers are also commenters and viewers.
- Using [**the union operator**](/docs/configuration-language#the-union-operator) condition to indicate that a user might have a certain relation with an object if they match any of the criteria indicated.
...
- Using the **[type bound public access](/docs/concepts#what-is-type-bound-public-access)** in a [relationship tuple's](/docs/concepts#what-is-a-relationship-tuple) user field to indicate that everyone has a certain relation with an object. See [Modeling Public Access](/docs/modeling/public-access) for more.
  Used here to [share documents publicly](#04-sharing-files-and-folders-publicly).
- Model [**parent-child objects**](/docs/modeling/parent-child) to indicate that a user having a relationship with a certain object implies having a relationship with another object in OpenFGA.
...
<h4 id="modeling-object-to-object-relationships">Modeling object-to-object relationships</h4>

You need to know how to create relationships between objects and how that might affect a user's relationships to those objects. [Learn more →](/docs/modeling/building-blocks/object-to-object-relationships)

Used here to indicate that users who have access to view a folder have access to view all documents inside it.

<h4 id="modeling-public-access">Modeling public access</h4>

You need to know how to add a relationship tuple to indicate that a resource is publicly available. [Learn more →](/docs/modeling/public-access)

<h4 id="concepts--configuration-language">Concepts & configuration language</h4>

- The [OpenFGA Concepts](/docs/concepts)
...
<h2 id="modeling-google-drives-permissions">Modeling Google Drive's permissions</h2>
...
To represent permissions in OpenFGA we use [relations](/docs/concepts#what-is-a-relation). For document permissions we need to create the following [authorization model](/docs/concepts#what-is-an-authorization-model):
...
This means that a user can be [directly related](/docs/concepts#what-are-direct-and-implied-relationships) as a viewer to an object of type "document"
</Note>
...
And [ask](/docs/concepts#what-is-a-check-request) some questions to OpenFGA:
...
![Diagram showing organization-wide Google Drive permissions](./assets/gdrive-org.svg)
...
To add support for domains and members all we need to do is add this object to the OpenFGA [authorization model](/docs/concepts#what-is-a-type-definition). In addition, update the model to allow domain member to be assigned to document:
...
The last relationship tuple introduces a new **OpenFGA** concept. A **[userset](/docs/concepts#what-is-a-user)**. When the value of a user is formatted like this **objectType:objectId#relation**, OpenFGA will automatically expand the userset into all its individual user identifiers:
...
More details on this technique can be found in the section [Modeling Parent-Child Objects](/docs/modeling/parent-child).
...
- the "viewers": the set of users who are [directly related](/docs/concepts#what-are-direct-and-implied-relationships) to the document as a "viewer"
...
Here's where another OpenFGA feature, [type bound public access](/docs/concepts#what-is-type-bound-public-access) (as in everyone), would come in handy.
