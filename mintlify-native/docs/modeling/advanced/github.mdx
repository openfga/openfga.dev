...
- Indicate relationships between a group of **users** and an **object**. See [Modeling User Groups](/docs/modeling/user-groups) for more details.
  Used here to indicate that all members of an organization are repository admins on the organization.
- Modeling **concentric relationship** to have a certain relation on an object imply another relation on the same object. See [Modeling Concepts: Concentric Relationships](/docs/modeling/building-blocks/concentric-relationships) for more.
...
Let's say we have a **protocols** team that is part of the **engineering**. The simplest way to achieve the aforementioned requirement is just adding this relationship tuple:
