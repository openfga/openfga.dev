...
In many cases, roles would fit in well as relations on an object type, as seen in [Modeling Roles and Permissions](/docs/modeling/roles-and-permissions). In some cases, however, they may not be enough.
...
To start, let's say there is an application with a [type](/docs/concepts#what-is-a-type) called `asset-category`. Users can have view and/or edit access to assets in that category. Any user who can edit can also view.

<Accordion title="Initial Model" id="initial-model">
...
<h3 id="modeling-roles-and-permissions">Modeling Roles and Permissions</h3>

You need to know how to add users to groups and grant groups access to resources. [Learn more →](/docs/modeling/user-groups)

<h3 id="modeling-object-to-object-relationships">Modeling Object-to-Object Relationships</h3>
...
<h3 id="concepts--configuration-language">Concepts & Configuration Language</h3>

- [OpenFGA Concepts](/docs/concepts)
...
At the end, we'll verify our model by ensuring the following access [check](/docs/concepts#what-is-a-check-request) requests return the expected result.
...
<h3 id="02use-relationship-tuples-to-tie-the-users-to-the-roles">02. Use Relationship Tuples To Tie The Users To The Roles</h3>
...
With our users and roles set up, we still need to tie members of a certain role to its corresponding permission(s).
