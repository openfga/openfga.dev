...
When checking for a relationship, a direct relationship exists if a _**relationship tuple**_ is present in the system with the exact same object and relation that were in the query and where the user is one of:
...
Direct relationships can be enabled for a specific relation on an _**object type**_ by adding [direct relationship type restrictions](/docs/configuration-language#direct-relationship-type-restrictions) from that **relation's definition**. Likewise, they can be disabled by removing the direct relationship type restrictions.
...
Assume you have a **type bound public access** tuple where everyone of type `user` is a `viewer` of `document:planning` (In other words, the document is public)
...
Here we will cover one example of an **indirect relationship** in order to see how they differ from direct relationships.
