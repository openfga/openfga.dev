...
- Are only supported on the [Check API endpoint](/api/service#Relationship%20Queries/Check) and [ListObjects API endpoint](/api/service#Relationship%20Queries/ListObjects). They are not supported on read, expand and other endpoints.
- If you are using the [Read Changes API endpoint](/api/service#Relationship%20Tuples/ReadChanges) to build a permission aware search index, note that it will not be trivial to take contextual tuples into account.
</Warning>
