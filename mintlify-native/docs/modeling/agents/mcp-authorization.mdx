...
type role
\x20 relations
\x20   define assignee: [user, group#member]
type tool
\x20 relations
\x20   define can_call: [user:*, user, role#assignee, user with temporal_grant, role#assignee with temporal_grant]
...
\x20 relations
\x20   define can_call: [user:*, user, role#assignee, user with temporal_grant, role#assignee with temporal_grant]
\x20   define can_view_private_documents: [role#assignee]
condition temporal_grant(grant_time: timestamp, grant_duration: duration, current_time: timestamp) {
\x20 current_time < grant_time + grant_duration
}`} />
