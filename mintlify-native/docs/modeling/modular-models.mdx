...
Modular models solves that problem by allowing individual types to be extended within other modules to share those relations.
...
<OpenFGACodeBlock code={`module core
...
<OpenFGACodeBlock code={`module issue-tracker
...
\x20   define viewer: member from organization`} />
...
<OpenFGACodeBlock code={`module wiki

extend type organization
\x20 relations
\x20   define can_create_space: admin


type space
\x20 relations
\x20   define organization: [organization]
\x20   define can_view_pages: member from organization

type page
\x20 relations
\x20   define space: [space]
\x20   define owner: [user]`} />

<h3 id="fgamod-1">`fga.mod`</h3>
...
For example, the `organization` type shows that the type is defined in the `core.fga` file as part of the `core` module, the `can_create_project` relation is defined in `issue-tracker/projects.fga` as part of the `issue-tracker` module, and the `can_create_space` relation is defined in the `wiki.fga` file as part of the `wiki` module.

<OpenFGACodeBlock code={`type organization # module: core, file: core.fga
\x20 relations
\x20   define admin: [user]
\x20   define member: [user] or admin
\x20   define can_create_project: admin # extended by: module: issue-tracker, file: issue-tracker/projects.fga
\x20   define can_create_space: admin # extended by: module: wiki, file: wiki.fga`} />
