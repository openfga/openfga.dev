...
The [Zanzibar paper](/docs/authorization-concepts#what-is-zanzibar) has a feature called `Zookies`, which is a consistency token that is returned from Write operation. You can store that token in your resource table, and specify it in subsequent calls to query APIs.
