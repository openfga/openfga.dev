...
The following [Relationship Tuple](/docs/concepts#what-is-a-relationship-tuple) assigns every `employee` of a type `company` a `reader` relationship with a particular object of type `document`, in this case `document:planning`:
