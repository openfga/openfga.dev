...
page. Mintlify renders an interactive API playground natively from the canonical
OpenAPI 3.0.3 document generated by `openfga/api`, fetched from its `main` branch.
Compatible sample updates are proposed through draft PRs; incompatible updates
are reported in issues. No separate package, local
conversion, or custom page is required.
